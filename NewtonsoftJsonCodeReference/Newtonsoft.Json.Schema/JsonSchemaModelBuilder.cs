...
internal class JsonSchemaModelBuilder
{
	...
	public void AddProperties(IDictionary<string, JsonSchema> source, IDictionary<string, JsonSchemaNode> target)
	{
		...
		{
			this.AddProperty(target, item.Key, item.Value);
		}
	}
	...
	public void AddProperty(IDictionary<string, JsonSchemaNode> target, string propertyName, JsonSchema schema)
	{
		...
		target[propertyName] = this.AddSchema(value, schema);
	}
	...
	{
		if (this._nodeModels.TryGetValue(node, out var value))
		{
			return value;
		}
		value = JsonSchemaModel.Create(node.Schemas);
		this._nodeModels[node] = value;
		foreach (KeyValuePair<string, JsonSchemaNode> property in node.Properties)
		{
			...
			value.Properties[property.Key] = this.BuildNodeModel(property.Value);
		}
		foreach (KeyValuePair<string, JsonSchemaNode> patternProperty in node.PatternProperties)
		{
			...
			value.PatternProperties[patternProperty.Key] = this.BuildNodeModel(patternProperty.Value);
		}
		foreach (JsonSchemaNode item in node.Items)
		{
			...
			value.Items.Add(this.BuildNodeModel(item));
		}
		if (node.AdditionalProperties != null)
		{
			value.AdditionalProperties = this.BuildNodeModel(node.AdditionalProperties);
		}
		if (node.AdditionalItems != null)
		{
			value.AdditionalItems = this.BuildNodeModel(node.AdditionalItems);
		}
		return value;
	}
}
