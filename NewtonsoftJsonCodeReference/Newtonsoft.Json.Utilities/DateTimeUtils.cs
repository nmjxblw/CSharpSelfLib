...
internal static class DateTimeUtils
{
	...
	static DateTimeUtils()
	{
		...
		DateTimeUtils.DaysToMonth365 = new int[13]
		{
			0, 31, 59, 90, 120, 151, 181, 212, 243, 273,
			304, 334, 365
		};
		DateTimeUtils.DaysToMonth366 = new int[13]
		{
	...
	{
		return DateTimeUtils.ConvertDateTimeToJavaScriptTicks(dateTime, convertToUtc: true);
	}
	...
	{
		return (universalTicks - DateTimeUtils.InitialJavaScriptDateTicks) / 10000;
	}

	internal static DateTime ConvertJavaScriptTicksToDateTime(long javaScriptTicks)
	{
		return new DateTime(javaScriptTicks * 10000 + DateTimeUtils.InitialJavaScriptDateTicks, DateTimeKind.Utc);
	}
	...
	{
		if (!DateTimeUtils.TryParseMicrosoftDate(text, out var ticks, out var _, out var kind))
		{
			dt = default(DateTime);
			return false;
		}
	...
	{
		if (!DateTimeUtils.TryParseMicrosoftDate(text, out var ticks, out var offset, out var _))
		{
			dt = default(DateTime);
			return false;
		}
		dt = new DateTimeOffset(DateTimeUtils.ConvertJavaScriptTicksToDateTime(ticks).Add(offset).Ticks, offset);
		return true;
	}
	...
	internal static int WriteDateTimeString(char[] chars, int start, DateTime value, TimeSpan? offset, DateTimeKind kind, DateFormatHandling format)
	{
		...
		if (format == DateFormatHandling.MicrosoftDateFormat)
		{
			...
			long num2 = DateTimeUtils.ConvertDateTimeToJavaScriptTicks(value, offset2);
			...
			case DateTimeKind.Unspecified:
				...
				{
					num = DateTimeUtils.WriteDateTimeOffset(chars, num, offset2, format);
				}
				break;
			case DateTimeKind.Local:
				num = DateTimeUtils.WriteDateTimeOffset(chars, num, offset2, format);
		...
		{
			num = DateTimeUtils.WriteDefaultIsoDate(chars, num, value);
			switch (kind)
			{
			case DateTimeKind.Local:
				num = DateTimeUtils.WriteDateTimeOffset(chars, num, offset ?? value.GetUtcOffset(), format);
				break;
	...
	{
		int num = 19;
		DateTimeUtils.GetDateValues(dt, out var year, out var month, out var day);
	...
	internal static void WriteDateTimeOffsetString(TextWriter writer, DateTimeOffset value, DateFormatHandling format, string? formatString, CultureInfo culture)
	{
		...
		{
			char[] array = new char[64];
			int count = DateTimeUtils.WriteDateTimeString(array, 0, (format == DateFormatHandling.IsoDateFormat) ? value.DateTime : value.UtcDateTime, value.Offset, DateTimeKind.Local, format);
			writer.Write(array, 0, count);
		}
		else
		{
	...
	private static void GetDateValues(DateTime td, out int year, out int month, out int day)
	{
		...
		num -= num5 * 365;
		int[] array = ((num5 == 3 && (num4 != 24 || num3 == 3)) ? DateTimeUtils.DaysToMonth366 : DateTimeUtils.DaysToMonth365);
		int i;
