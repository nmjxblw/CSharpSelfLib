...
internal static class JavaScriptUtils
{
	...
	static JavaScriptUtils()
	{
		...
		{
			JavaScriptUtils.SingleQuoteCharEscapeFlags[(uint)item] = true;
		}
		foreach (char item2 in list.Union(new char[1] { '"' }))
		{
			JavaScriptUtils.DoubleQuoteCharEscapeFlags[(uint)item2] = true;
		}
		foreach (char item3 in list.Union(new char[5] { '"', '\'', '<', '>', '&' }))
		{
			JavaScriptUtils.HtmlCharEscapeFlags[(uint)item3] = true;
		}
	}
	...
	public static string ToEscapedJavaScriptString(string? value, char delimiter, bool appendDelimiters, StringEscapeHandling stringEscapeHandling)
	{
		...
		JavaScriptUtils.WriteEscapedJavaScriptString(stringWriter, value, delimiter, appendDelimiters, charEscapeFlags, stringEscapeHandling, null, ref writeBuffer);
	...
	private static async Task WriteEscapedJavaScriptStringWithDelimitersAsync(Task task, TextWriter writer, string s, char delimiter, bool[] charEscapeFlags, StringEscapeHandling stringEscapeHandling, JsonTextWriter client, char[] writeBuffer, CancellationToken cancellationToken)
	{
		...
		if (!StringUtils.IsNullOrEmpty(s))
		{
			await JavaScriptUtils.WriteEscapedJavaScriptStringWithoutDelimitersAsync(writer, s, charEscapeFlags, stringEscapeHandling, client, writeBuffer, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
		}
	...
	public static bool TryGetDateFromConstructorJson(JsonReader reader, out DateTime dateTime, [NotNullWhen(false)] out string? errorMessage)
	{
		...
		errorMessage = null;
		if (!JavaScriptUtils.TryGetDateConstructorValue(reader, out var integer, out errorMessage) || !integer.HasValue)
		{
			errorMessage = errorMessage ?? "Date constructor has no arguments.";
			return false;
		}
		if (!JavaScriptUtils.TryGetDateConstructorValue(reader, out var integer2, out errorMessage))
		{
			return false;
		}
		if (integer2.HasValue)
		{
			...
			while (true)
			{
				if (!JavaScriptUtils.TryGetDateConstructorValue(reader, out var integer3, out errorMessage))
				{
					return false;
				}
