...
namespace Dopamine
{
    ...
    public static partial class ExtensionMethods
    {
        ...
        /// <returns></returns>
        public static string TolittleEndianString(this float inputFloat, int bytesCount = 3)
        {
            if (inputFloat == default) return string.Empty;
            int roundNumber = (int)Math.Round(inputFloat);
            bytesCount = Math.Max(1, bytesCount);
            // 为化整值限定一个范围
            int limit = bytesCount < 4 ? (1 << (8 * bytesCount)) - 1 : int.MaxValue;
            ...
            Array.Copy(temp, result, bytesCount);
            // 组帧，字符串表示
