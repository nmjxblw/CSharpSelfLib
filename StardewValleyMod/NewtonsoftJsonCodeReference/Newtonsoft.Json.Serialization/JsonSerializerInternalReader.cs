...
internal class JsonSerializerInternalReader : JsonSerializerInternalBase
{
	...
	public void Populate(JsonReader reader, object target)
	{
		...
		JsonContract jsonContract = base.Serializer._contractResolver.ResolveContract(type);
		...
		if (reader.TokenType == JsonToken.StartArray)
		{
			if (jsonContract.ContractType == JsonContractType.Array)
			{
				...
				this.PopulateList((IList)list, reader, jsonArrayContract, null, null);
				return;
			}
		...
		if (reader.TokenType == JsonToken.StartObject)
		{
			...
			string id = null;
			if (base.Serializer.MetadataPropertyHandling != MetadataPropertyHandling.Ignore && reader.TokenType == JsonToken.PropertyName && string.Equals(reader.Value.ToString(), "$id", StringComparison.Ordinal))
			{
				reader.ReadAndAssert();
			...
			if (jsonContract.ContractType == JsonContractType.Dictionary)
			{
				...
				this.PopulateDictionary((IDictionary)dictionary, reader, jsonDictionaryContract, null, id);
			}
			else
			{
	...
	{
		return base.Serializer._contractResolver.ResolveContract(type);
	}
	...
	public object? Deserialize(JsonReader reader, Type? objectType, bool checkAdditionalContent)
	{
		...
		try
		{
			...
			object result = ((converter == null || !converter.CanRead) ? this.CreateValueInternal(reader, objectType, contractSafe, null, null, null, null) : this.DeserializeConvertable(converter, reader, objectType, null));
		...
		catch (Exception ex)
		{
			...
			{
				this.HandleError(reader, readPastError: false, 0);
				return null;
			}
	...
	private object? CreateObject(JsonReader reader, Type? objectType, JsonContract? contract, JsonProperty? member, JsonContainerContract? containerContract, JsonProperty? containerMember, object? existingValue)
	{
		...
		string id;
		if (base.Serializer.MetadataPropertyHandling == MetadataPropertyHandling.Ignore)
		{
			reader.ReadAndAssert();
			id = null;
		}
		else if (base.Serializer.MetadataPropertyHandling == MetadataPropertyHandling.ReadAhead)
		{
			...
			if (this.ReadMetadataPropertiesToken(jTokenReader, ref objectType2, ref contract, member, containerContract, containerMember, existingValue, out object newValue, out id))
			{
				return newValue;
			}
		}
		...
		{
			reader.ReadAndAssert();
			if (this.ReadMetadataProperties(reader, ref objectType2, ref contract, member, containerContract, containerMember, existingValue, out object newValue2, out id))
			{
				return newValue2;
			}
		}
		...
		case JsonContractType.Object:
		{
			...
			object obj = ((existingValue == null || (!(objectType2 == objectType) && !objectType2.IsAssignableFrom(existingValue.GetType()))) ? this.CreateNewObject(reader, jsonObjectContract, member, containerMember, id, out createdFromNonDefaultCreator2) : existingValue);
			if (createdFromNonDefaultCreator2)
			{
				return obj;
			}
			return this.PopulateObject(obj, reader, jsonObjectContract, member, id);
		}
		case JsonContractType.Primitive:
		{
			JsonPrimitiveContract contract4 = (JsonPrimitiveContract)contract;
			if (base.Serializer.MetadataPropertyHandling != MetadataPropertyHandling.Ignore && reader.TokenType == JsonToken.PropertyName && string.Equals(reader.Value.ToString(), "$value", StringComparison.Ordinal))
			{
				reader.ReadAndAssert();
				if (reader.TokenType == JsonToken.StartObject)
				{
				...
				object? result = this.CreateValueInternal(reader, objectType2, contract4, member, null, null, existingValue);
				reader.ReadAndAssert();
				return result;
			}
			break;
		...
		case JsonContractType.Dictionary:
		{
			...
			if (existingValue == null)
			{
				...
				IDictionary dictionary = this.CreateNewDictionary(reader, jsonDictionaryContract, out createdFromNonDefaultCreator);
				...
				this.PopulateDictionary(dictionary, reader, jsonDictionaryContract, member, id);
				if (createdFromNonDefaultCreator)
				{
			...
			return this.PopulateDictionary((IDictionary)dictionary2, reader, jsonDictionaryContract, member, id);
		}
		case JsonContractType.Dynamic:
		{
		...
		format = format.FormatWith(CultureInfo.InvariantCulture, objectType2, this.GetExpectedDescription(contract));
		...
		if (reader.TokenType == JsonToken.StartObject)
		{
			...
			if (jProperty != null)
			{
				...
				if (text != null)
				{
					...
					newValue = base.Serializer.GetReferenceResolver().ResolveReference(this, text);
					if (base.TraceWriter != null && base.TraceWriter.LevelFilter >= TraceLevel.Info)
					{
						base.TraceWriter.Trace(TraceLevel.Info, JsonPosition.FormatMessage(reader, reader.Path, "Resolved object reference '{0}' to {1}.".FormatWith(CultureInfo.InvariantCulture, text, newValue.GetType())), null);
					}
					reader.Skip();
					return true;
				}
				...
				do
				{
					...
					if (string.Equals(text, "$ref", StringComparison.Ordinal))
					{
						...
						if (text2 != null)
						{
							...
							newValue = base.Serializer.GetReferenceResolver().ResolveReference(this, text2);
							if (base.TraceWriter != null && base.TraceWriter.LevelFilter >= TraceLevel.Info)
							{
								base.TraceWriter.Trace(TraceLevel.Info, JsonPosition.FormatMessage(reader as IJsonLineInfo, reader.Path, "Resolved object reference '{0}' to {1}.".FormatWith(CultureInfo.InvariantCulture, text2, newValue.GetType())), null);
							}
							return true;
						}
						flag = true;
	...
	{
		if ((member?.TypeNameHandling ?? containerContract?.ItemTypeNameHandling ?? containerMember?.ItemTypeNameHandling ?? base.Serializer._typeNameHandling) != TypeNameHandling.None)
		{
			StructMultiKey<string, string> structMultiKey = ReflectionUtils.SplitFullyQualifiedTypeName(qualifiedTypeName);
			Type type;
			try
			{
				type = base.Serializer._serializationBinder.BindToType(structMultiKey.Value1, structMultiKey.Value2);
			}
			catch (Exception ex)
			{
			...
			{
				base.TraceWriter.Trace(TraceLevel.Verbose, JsonPosition.FormatMessage(reader as IJsonLineInfo, reader.Path, "Resolved type '{0}' to {1}.".FormatWith(CultureInfo.InvariantCulture, qualifiedTypeName, type)), null);
			}
	...
	private JsonArrayContract EnsureArrayContract(JsonReader reader, Type objectType, JsonContract contract)
	{
		...
		if (obj == null)
		{
			...
			format = format.FormatWith(CultureInfo.InvariantCulture, objectType, this.GetExpectedDescription(contract));
			throw JsonSerializationException.Create(reader, format);
		}
		return obj;
	}
	...
	private object? CreateList(JsonReader reader, Type? objectType, JsonContract? contract, JsonProperty? member, object? existingValue, string? id)
	{
		...
		{
			bool createdFromNonDefaultCreator;
			IList list = this.CreateNewList(reader, jsonArrayContract, out createdFromNonDefaultCreator);
			if (createdFromNonDefaultCreator)
			{
				if (id != null)
				{
		...
		return this.PopulateList(list3, reader, jsonArrayContract, member, id);
	}
	...
	{
		if (this.CalculatePropertyDetails(property, ref propertyConverter, containerContract, containerProperty, reader, target, out bool useExistingValue, out object currentValue, out JsonContract propertyContract, out bool gottenCurrentValue, out bool ignoredValue))
		{
			if (ignoredValue)
			{
				return true;
		...
		{
			obj = this.CreateValueInternal(reader, property.PropertyType, propertyContract, property, containerContract, containerProperty, useExistingValue ? currentValue : null);
		}
	...
	private bool CalculatePropertyDetails(JsonProperty property, ref JsonConverter? propertyConverter, JsonContainerContract? containerContract, JsonProperty? containerProperty, JsonReader reader, object target, out bool useExistingValue, out object? currentValue, out JsonContract? propertyContract, out bool gottenCurrentValue, out bool ignoredValue)
	{
		...
		if (property.ObjectCreationHandling.GetValueOrDefault(base.Serializer._objectCreationHandling) != ObjectCreationHandling.Replace && (tokenType == JsonToken.StartArray || tokenType == JsonToken.StartObject || propertyConverter != null) && property.Readable)
		{
		...
		if (!property.Writable && !useExistingValue)
		{
			if (base.TraceWriter != null && base.TraceWriter.LevelFilter >= TraceLevel.Info)
			{
				base.TraceWriter.Trace(TraceLevel.Info, JsonPosition.FormatMessage(reader as IJsonLineInfo, reader.Path, "Unable to deserialize value to non-writable property '{0}' on {1}.".FormatWith(CultureInfo.InvariantCulture, property.PropertyName, property.DeclaringType)), null);
			}
			return true;
		}
	...
	private void AddReference(JsonReader reader, string id, object value)
	{
		try
		{
			...
			base.Serializer.GetReferenceResolver().AddReference(this, id, value);
		}
		catch (Exception ex)
		{
	...
	private IList CreateNewList(JsonReader reader, JsonArrayContract contract, out bool createdFromNonDefaultCreator)
	{
		...
		if (contract.DefaultCreator != null && (!contract.DefaultCreatorNonPublic || base.Serializer._constructorHandling == ConstructorHandling.AllowNonPublicDefaultConstructor))
		{
	...
	private void OnDeserializing(JsonReader reader, JsonContract contract, object value)
	{
		...
		contract.InvokeOnDeserializing(value, base.Serializer._context);
	}
	...
	private void OnDeserialized(JsonReader reader, JsonContract contract, object value)
	{
		...
		contract.InvokeOnDeserialized(value, base.Serializer._context);
	}
			...
			case JsonToken.PropertyName:
			{
				...
				try
				{
					try
					{
						...
						{
							obj2 = (DateTimeUtils.TryParseDateTime(obj2.ToString(), reader.DateTimeZoneHandling, reader.DateFormatString, reader.Culture, out var dt2) ? ((object)dt2) : this.EnsureType(reader, obj2, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType));
							break;
						}
						case PrimitiveTypeCode.DateTimeOffset:
						case PrimitiveTypeCode.DateTimeOffsetNullable:
						{
							obj2 = (DateTimeUtils.TryParseDateTimeOffset(obj2.ToString(), reader.DateFormatString, reader.Culture, out var dt) ? ((object)dt) : this.EnsureType(reader, obj2, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType));
							break;
						}
						default:
							obj2 = ((contract.KeyContract != null && contract.KeyContract.IsEnum) ? EnumUtils.ParseEnum(contract.KeyContract.NonNullableUnderlyingType, (base.Serializer._contractResolver as DefaultContractResolver)?.NamingStrategy, obj2.ToString(), disallowNumber: false) : this.EnsureType(reader, obj2, CultureInfo.InvariantCulture, contract.KeyContract, contract.DictionaryKeyType));
							break;
						}
					}
					catch (Exception ex)
					...
					object value = ((jsonConverter == null || !jsonConverter.CanRead) ? this.CreateValueInternal(reader, contract.DictionaryValueType, contract.ItemContract, null, contract, containerProperty, null) : this.DeserializeConvertable(jsonConverter, reader, contract.DictionaryValueType, null));
				...
				catch (Exception ex2)
				{
					...
					{
						this.HandleError(reader, readPastError: true, depth);
						break;
					}
					throw;
				}
			...
			if (stack.Count == arrayRank)
			{
				try
				{
					if (reader.ReadForType(contractSafe, converter != null))
					{
						...
						default:
						{
							object value = ((converter == null || !converter.CanRead) ? this.CreateValueInternal(reader, contract.CollectionItemType, contractSafe, null, contract, containerProperty, null) : this.DeserializeConvertable(converter, reader, contract.CollectionItemType, null));
							list2.Add(value);
							break;
						}
						case JsonToken.Comment:
				...
				catch (Exception ex)
				{
					...
					{
						this.HandleError(reader, readPastError: true, depth + 1);
		...
		do
		{
			try
			{
				if (reader.ReadForType(contract.ItemContract, converter != null))
				{
					...
					default:
					{
						object value = ((converter == null || !converter.CanRead) ? this.CreateValueInternal(reader, contract.CollectionItemType, contract.ItemContract, null, contract, containerProperty, null) : this.DeserializeConvertable(converter, reader, contract.CollectionItemType, null));
						list.Add(value);
						break;
					}
					case JsonToken.Comment:
	...
	private object CreateISerializable(JsonReader reader, JsonISerializableContract contract, JsonProperty? member, string? id)
	{
		...
		object obj = contract.ISerializableCreator(serializationInfo, base.Serializer._context);
		if (id != null)
		{
			this.AddReference(reader, id, obj);
		}
	...
	internal object? CreateISerializableItem(JToken token, Type type, JsonISerializableContract contract, JsonProperty? member)
	{
		...
		jsonReader.MaxDepth = base.Serializer.MaxDepth;
		jsonReader.ReadAndAssert();
		if (converter != null && converter.CanRead)
		{
			return this.DeserializeConvertable(converter, jsonReader, type, null);
		}
		return this.CreateValueInternal(jsonReader, type, contractSafe, null, contract, member, null);
	}
				...
				case JsonToken.PropertyName:
				{
					...
					try
					{
						...
						else
						{
							...
							object value = ((converter2 == null || !converter2.CanRead) ? this.CreateValueInternal(reader, type, contractSafe, null, null, member, null) : this.DeserializeConvertable(converter2, reader, type, null));
	...
	private object CreateObjectUsingCreatorWithParameters(JsonReader reader, JsonObjectContract contract, JsonProperty? containerProperty, ObjectConstructor<object> creator, string? id)
	{
		...
		bool flag = contract.HasRequiredOrDefaultValueProperties || this.HasFlag(base.Serializer._defaultValueHandling, DefaultValueHandling.Populate);
		Type underlyingType = contract.UnderlyingType;
		if (base.TraceWriter != null && base.TraceWriter.LevelFilter >= TraceLevel.Info)
		{
			string arg = string.Join(", ", contract.CreatorParameters.Select((JsonProperty p) => p.PropertyName));
			base.TraceWriter.Trace(TraceLevel.Info, JsonPosition.FormatMessage(reader as IJsonLineInfo, reader.Path, "Deserializing {0} using creator with parameters: {1}.".FormatWith(CultureInfo.InvariantCulture, contract.UnderlyingType, arg)), null);
		}
		List<CreatorPropertyContext> list = this.ResolvePropertyAndCreatorValues(contract, containerProperty, reader, underlyingType);
		if (flag)
		{
		...
		foreach (CreatorPropertyContext item in list)
		{
			if (flag && item.Property != null && !item.Presence.HasValue)
			{
				...
				PropertyPresence value2 = ((value == null) ? PropertyPresence.Null : ((!(value is string s)) ? PropertyPresence.Value : (JsonSerializerInternalReader.CoerceEmptyStringToNull(item.Property.PropertyType, item.Property.PropertyContract, s) ? PropertyPresence.Null : PropertyPresence.Value)));
			...
			if (flag && (item.Presence == PropertyPresence.None || item.Presence == PropertyPresence.Null))
			{
				...
				if (this.HasFlag(jsonProperty.DefaultValueHandling.GetValueOrDefault(base.Serializer._defaultValueHandling), DefaultValueHandling.Populate))
				{
					item.Value = this.EnsureType(reader, jsonProperty.GetResolvedDefaultValue(), CultureInfo.InvariantCulture, jsonProperty.PropertyContract, jsonProperty.PropertyType);
				}
			}
		...
		foreach (CreatorPropertyContext item2 in list)
		{
			...
			else
			{
				...
				JsonContract jsonContract = base.Serializer._contractResolver.ResolveContract(property2.PropertyType);
				if (jsonContract.ContractType == JsonContractType.Array)
				{
					JsonArrayContract jsonArrayContract = (JsonArrayContract)jsonContract;
		...
		if (flag)
		{
			foreach (CreatorPropertyContext item6 in list)
			{
				...
				{
					this.EndProcessProperty(obj, reader, contract, reader.Depth, item6.Property, item6.Presence.GetValueOrDefault(), !item6.Used);
				}
			}
		}
		this.OnDeserialized(reader, contract, obj);
		...
		do
		{
			...
			case JsonToken.PropertyName:
			{
				...
				else
				{
					...
					if (base.TraceWriter != null && base.TraceWriter.LevelFilter >= TraceLevel.Verbose)
					{
						base.TraceWriter.Trace(TraceLevel.Verbose, JsonPosition.FormatMessage(reader as IJsonLineInfo, reader.Path, "Could not find member '{0}' on {1}.".FormatWith(CultureInfo.InvariantCulture, text, contract.UnderlyingType)), null);
					}
					if ((contract.MissingMemberHandling ?? base.Serializer._missingMemberHandling) == MissingMemberHandling.Error)
					{
	...
	public object CreateNewObject(JsonReader reader, JsonObjectContract objectContract, JsonProperty? containerMember, JsonProperty? containerProperty, string? id, out bool createdFromNonDefaultCreator)
	{
		...
		else if (objectContract.DefaultCreator != null && (!objectContract.DefaultCreatorNonPublic || base.Serializer._constructorHandling == ConstructorHandling.AllowNonPublicDefaultConstructor || objectContract.ParameterizedCreator == null))
		{
	...
	private object PopulateObject(object newObject, JsonReader reader, JsonObjectContract contract, JsonProperty? member, string? id)
	{
		...
		Dictionary<JsonProperty, PropertyPresence> dictionary = ((contract.HasRequiredOrDefaultValueProperties || this.HasFlag(base.Serializer._defaultValueHandling, DefaultValueHandling.Populate)) ? contract.Properties.ToDictionary((JsonProperty m) => m, (JsonProperty m) => PropertyPresence.None) : null);
		if (id != null)
		{
		...
		if (dictionary != null)
		{
			foreach (KeyValuePair<JsonProperty, PropertyPresence> item in dictionary)
			{
				...
				this.EndProcessProperty(newObject, reader, contract, depth, key, value, setDefaultValue: true);
			}
		}
	...
	private bool ShouldDeserialize(JsonReader reader, JsonProperty property, object target)
	{
		...
		if (base.TraceWriter != null && base.TraceWriter.LevelFilter >= TraceLevel.Verbose)
		{
			base.TraceWriter.Trace(TraceLevel.Verbose, JsonPosition.FormatMessage(null, reader.Path, "ShouldDeserialize result for property '{0}' on {1}: {2}".FormatWith(CultureInfo.InvariantCulture, property.PropertyName, property.DeclaringType, flag)), null);
		}
		return flag;
	}
	...
	{
		if (base.Serializer.MetadataPropertyHandling == MetadataPropertyHandling.ReadAhead)
		{
			switch (memberName)
			{
			case "$id":
	...
	private object? ReadExtensionDataValue(JsonObjectContract contract, JsonProperty? member, JsonReader reader)
	{
		...
		return this.CreateValueInternal(reader, null, null, null, contract, member, null);
	}
	...
	private void EndProcessProperty(object newObject, JsonReader reader, JsonObjectContract contract, int initialDepth, JsonProperty property, PropertyPresence presence, bool setDefaultValue)
	{
		...
		try
		{
			...
			case PropertyPresence.None:
				...
				if (setDefaultValue && !property.Ignored)
				{
					...
					if (this.HasFlag(property.DefaultValueHandling.GetValueOrDefault(base.Serializer._defaultValueHandling), DefaultValueHandling.Populate) && property.Writable)
					{
		...
		catch (Exception ex)
		{
			...
			{
				this.HandleError(reader, readPastError: true, initialDepth);
				return;
			}
			throw;
		}
	...
	private void SetPropertyPresence(JsonReader reader, JsonProperty property, Dictionary<JsonProperty, PropertyPresence>? requiredProperties)
	{
		if (property != null && requiredProperties != null)
		{
			...
			{
			case JsonToken.String:
				value = (JsonSerializerInternalReader.CoerceEmptyStringToNull(property.PropertyType, property.PropertyContract, (string)reader.Value) ? PropertyPresence.Null : PropertyPresence.Value);
				break;
			case JsonToken.Null:
			case JsonToken.Undefined:
