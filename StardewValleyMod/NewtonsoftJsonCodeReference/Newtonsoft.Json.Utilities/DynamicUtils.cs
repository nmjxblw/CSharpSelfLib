	...
	internal static class BinderWrapper
	{
		...
		private static void Init()
		{
			if (!BinderWrapper._init)
			{
				...
				BinderWrapper._getCSharpArgumentInfoArray = BinderWrapper.CreateSharpArgumentInfoArray(default(int));
				BinderWrapper._setCSharpArgumentInfoArray = BinderWrapper.CreateSharpArgumentInfoArray(0, 3);
				BinderWrapper.CreateMemberCalls();
				BinderWrapper._init = true;
			}
		}
		...
		private static void CreateMemberCalls()
		{
			...
			BinderWrapper._getMemberCall = JsonTypeReflector.ReflectionDelegateFactory.CreateMethodCall<object>(method);
			...
			BinderWrapper._setMemberCall = JsonTypeReflector.ReflectionDelegateFactory.CreateMethodCall<object>(method2);
		}

		public static CallSiteBinder GetMember(string name, Type context)
		{
			BinderWrapper.Init();
			return (CallSiteBinder)BinderWrapper._getMemberCall(null, 0, name, context, BinderWrapper._getCSharpArgumentInfoArray);
		}

		public static CallSiteBinder SetMember(string name, Type context)
		{
			BinderWrapper.Init();
			return (CallSiteBinder)BinderWrapper._setMemberCall(null, 0, name, context, BinderWrapper._setCSharpArgumentInfoArray);
		}
	}
