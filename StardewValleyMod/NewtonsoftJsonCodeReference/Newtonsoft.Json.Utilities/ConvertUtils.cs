...
internal static class ConvertUtils
{
	...
	{
		if (ConvertUtils.TypeCodeMap.TryGetValue(t, out var value))
		{
			isEnum = false;
			return value;
		}
	...
	{
		object value;
		return ConvertUtils.TryConvertInternal(initialValue, culture, targetType, out value) switch
		{
	...
	private static ConvertResult TryConvertInternal(object? initialValue, CultureInfo culture, Type targetType, out object? value)
	{
		...
		if (initialValue is string text)
		{
			...
			{
				if (ConvertUtils.VersionTryParse(text, out Version result))
				{
					value = result;
					return ConvertResult.Success;
				}
			...
			{
				value = TimeOnly.ParseExact(text, "HH':'mm':'ss.FFFFFFF", CultureInfo.InvariantCulture);
				return ConvertResult.Success;
			}
		}
	...
	public static object? ConvertOrCast(object? initialValue, CultureInfo culture, Type targetType)
	{
		...
		if (ConvertUtils.TryConvert(initialValue, culture, targetType, out object value))
		{
			return value;
		}
