...
public class TrainCar : INetObject<NetFields>
{
	...
	{
		this.NetFields.SetOwner(this).AddField(this.frontDecal, "frontDecal").AddField(this.carType, "carType")
			.AddField(this.resourceType, "resourceType")
			.AddField(this.loaded, "loaded")
			.AddField(this.topFeatures, "topFeatures")
			.AddField(this.alternateCar, "alternateCar")
			.AddField(this.color, "color");
	}

	public void draw(SpriteBatch b, Vector2 globalPosition, float wheelRotation, GameLocation location)
	{
		b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition), new Rectangle(192 + this.carType.Value * 128, 512 - (this.alternateCar.Value ? 64 : 0), 128, 57), this.color.Value, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 256f) / 10000f);
		b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition + new Vector2(0f, 228f)), new Rectangle(192 + this.carType.Value * 128, 569, 128, 7), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 256f) / 10000f);
		switch (this.carType.Value)
		{
		case 1:
			b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition), new Rectangle(448 + this.resourceType.Value * 128 % 256, 576 + this.resourceType.Value / 2 * 32, 128, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 260f) / 10000f);
			if (this.loaded.Value > 0 && Game1.random.NextDouble() < 0.02 && globalPosition.X > 320f && globalPosition.X < (float)(location.map.DisplayWidth - 256))
			{
				...
				case 1:
					...
					break;
				case 7:
					debrisId = (location.IsWinterHere() ? "(O)536" : ((Game1.stats.DaysPlayed > 120 && this.color.R > this.color.G) ? "(O)537" : "(O)535"));
					break;
				case 2:
		...
		case 0:
		{
			for (int i = 0; i < this.topFeatures.Count; i += 64)
			{
				...
				{
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition + new Vector2(64 + i, 20f)), new Rectangle(192, 608 + this.topFeatures[i] * 16, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 260f) / 10000f);
				}
			}
			this.DrawFrontDecal(b, globalPosition);
			break;
	...
	private void DrawFrontDecal(SpriteBatch b, Vector2 globalPosition)
	{
		...
		else if (this.frontDecal.Value != -1 && this.frontDecal.Value < 35)
		{
			b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, globalPosition + new Vector2(192f, 92f)), new Rectangle(224 + this.frontDecal.Value * 32 % 224, 576 + this.frontDecal.Value * 32 / 224 * 32, 32, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (globalPosition.Y + 260f) / 10000f);
		}
	}
}
