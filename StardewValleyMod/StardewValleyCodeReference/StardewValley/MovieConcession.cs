...
public class MovieConcession : ISalable, IHaveItemTypeId
{
	...
	{
		this.Data = data;
		this.Tags = data.ItemTags?.ToList();
	}
	...
	public void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, StackDrawType drawStackNumber, Color color, bool drawShadow)
	{
		...
		spriteBatch.Draw(this.GetTexture(), location + new Vector2((int)(32f * scaleSize), (int)(32f * scaleSize)), Game1.getSourceRectForStandardTileSheet(this.GetTexture(), this.GetSpriteIndex(), 16, 16), color * transparency, 0f, new Vector2(8f, 8f) * scaleSize, 4f * scaleSize, SpriteEffects.None, layerDepth);
	}
