...
public static class ArgUtility
{
	...
	{
		if (!ArgUtility.TryGet(array, index, out var raw, out error, allowBlank: false, name))
		{
			value = false;
			return false;
		}
		...
		{
			value = false;
			error = ArgUtility.GetValueParseError(array, index, name, required: true, "a boolean (should be 'true' or 'false')");
			return false;
		}
		error = null;
		return true;
	...
	public static bool TryGetOptionalBool(string[] array, int index, out bool value, out string error, bool defaultValue = false, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			error = ArgUtility.GetValueParseError(array, index, name, required: false, "a boolean");
			value = defaultValue;
			return false;
		}
		error = null;
	...
	public static bool TryGetDirection(string[] array, int index, out int value, out string error, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			value = 0;
			error = ArgUtility.GetValueParseError(array, index, name, required: true, "a direction (should be 'up', 'down', 'left', or 'right')");
			return false;
		}
		error = null;
		return true;
	...
	public static bool TryGetOptionalDirection(string[] array, int index, out int value, out string error, int defaultValue = 0, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			error = ArgUtility.GetValueParseError(array, index, name, required: true, "a direction (should be one of 'up', 'down', 'left', or 'right')");
			value = defaultValue;
			return false;
		}
		error = null;
	...
	public static bool TryGetEnum<TEnum>(string[] array, int index, out TEnum value, out string error, [CallerArgumentExpression("value")] string name = null) where TEnum : struct
	{
		...
		if (!Utility.TryParseEnum<TEnum>(raw, out value))
		{
			...
			value = default(TEnum);
			error = ArgUtility.GetValueParseError(array, index, name, required: true, $"an enum of type '{type.FullName ?? type.Name}' (should be one of {string.Join(", ", Enum.GetNames(typeof(TEnum)))})");
			return false;
		}
		error = null;
		return true;
	...
	public static bool TryGetOptionalEnum<TEnum>(string[] array, int index, out TEnum value, out string error, TEnum defaultValue = default(TEnum), [CallerArgumentExpression("value")] string name = null) where TEnum : struct
	{
		...
		{
			Type type = typeof(TEnum);
			error = ArgUtility.GetValueParseError(array, index, name, required: false, "an enum of type '" + (type.FullName ?? type.Name) + "'");
			value = defaultValue;
			return false;
		}
		error = null;
	...
	public static bool TryGetFloat(string[] array, int index, out float value, out string error, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			value = 0f;
			error = ArgUtility.GetValueParseError(array, index, name, required: true, "a number");
			return false;
		}
		error = null;
		return true;
	...
	public static bool TryGetOptionalFloat(string[] array, int index, out float value, out string error, float defaultValue = 0f, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			error = ArgUtility.GetValueParseError(array, index, name, required: false, "a float");
			value = defaultValue;
			return false;
		}
		error = null;
	...
	public static bool TryGetInt(string[] array, int index, out int value, out string error, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			value = 0;
			error = ArgUtility.GetValueParseError(array, index, name, required: true, "an integer");
			return false;
		}
		error = null;
		return true;
	...
	public static bool TryGetOptionalInt(string[] array, int index, out int value, out string error, int defaultValue = 0, [CallerArgumentExpression("value")] string name = null)
	{
		...
		{
			error = ArgUtility.GetValueParseError(array, index, name, required: false, "an integer");
			value = defaultValue;
			return false;
		}
		error = null;
	...
	{
		if (!ArgUtility.TryGetInt(array, index, out var x, out error, (name != null) ? (name + " > x") : null) || !ArgUtility.TryGetInt(array, index + 1, out var y, out error, (name != null) ? (name + " > y") : null))
		{
			value = Point.Zero;
			return false;
		}
	...
	{
		if (!ArgUtility.TryGetInt(array, index, out var x, out error, (name != null) ? (name + " > x") : null) || !ArgUtility.TryGetInt(array, index + 1, out var y, out error, (name != null) ? (name + " > y") : null) || !ArgUtility.TryGetInt(array, index + 2, out var width, out error, (name != null) ? (name + " > width") : null) || !ArgUtility.TryGetInt(array, index + 3, out var height, out error, (name != null) ? (name + " > height") : null))
		{
	...
	public static bool TryGetVector2(string[] array, int index, out Vector2 value, out string error, bool integerOnly = false, [CallerArgumentExpression("value")] string name = null)
	{
		...
		if (integerOnly)
		{
			if (ArgUtility.TryGetInt(array, index, out var x, out error, xName) && ArgUtility.TryGetInt(array, index + 1, out var y, out error, yName))
			{
	...
	internal static string GetMissingRequiredIndexError(string[] array, int index, string name)
	{
		...
		{
			0 => "required " + ArgUtility.GetFieldLabel(index, name) + " not found (list is empty)", 
			1 => "required " + ArgUtility.GetFieldLabel(index, name) + " not found (list has a single value at index 0)", 
			_ => $"required {ArgUtility.GetFieldLabel(index, name)} not found (list has indexes 0 through {array.Length - 1})", 
		};
	}
	...
	{
		return $"{(required ? "required" : "optional")} {ArgUtility.GetFieldLabel(index, name)} has value '{array[index]}', which can't be parsed as {typeSummary}";
	}
