...
public class Torch : Object
{
	...
	{
		base.ItemId = base.parentSheetIndex.Value.ToString();
	}
	...
	public override void actionOnPlayerEntry()
	{
		...
		if (base.bigCraftable.Value && base.isOn.Value)
		{
			AmbientLocationSounds.addSound(base.tileLocation.Value, 1);
		}
	}
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		if (base.bigCraftable.Value)
		{
			...
			else if (base.bigCraftable.Value)
			{
	...
	public override bool placementAction(GameLocation location, int x, int y, Farmer who)
	{
		...
		Torch toPlace = (base.bigCraftable.Value ? new Torch(base.ItemId, bigCraftable: true) : new Torch(1, base.ItemId));
		if (base.bigCraftable.Value)
		{
	...
	{
		return !base.bigCraftable.Value;
	}
	...
	public override void updateWhenCurrentLocation(GameTime time)
	{
		...
		this.updateAshes((int)(base.tileLocation.X * 2000f + base.tileLocation.Y));
		...
		if (this.smokePuffTimer <= 0f)
		{
			...
			if (base.QualifiedItemId == "(BC)278")
			{
				Utility.addSmokePuff(environment, base.tileLocation.Value * 64f + new Vector2(32f, -32f));
			}
		}
	}
	...
	{
		if (!Utility.isOnScreen(base.tileLocation.Value * 64f, 256))
		{
			return;
		}
		for (int i = this.ashes.Length - 1; i >= 0; i--)
		{
			...
			if (i % 2 != 0)
			{
				temp.X += (float)Math.Sin((double)this.ashes[i].Y / (Math.PI * 2.0)) / 2f;
			}
			this.ashes[i] = temp;
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (!base.bigCraftable.Value)
		{
			...
			sourceRect.Y += 8;
			sourceRect.Height /= 2;
			spriteBatch.Draw(itemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64, y * 64 + 32)), sourceRect, Color.White, 0f, Vector2.Zero, (base.scale.Y > 1f) ? this.getScale().Y : 4f, SpriteEffects.None, (float)(bounds.Center.Y - 16) / 10000f);
			...
			{
				spriteBatch.Draw(Game1.objectSpriteSheet, Game1.GlobalToLocal(Game1.viewport, new Vector2((float)(x * 64 + 32) + this.ashes[i].X, (float)(y * 64 + 32) + this.ashes[i].Y)), new Rectangle(344 + i % 3, 53, 1, 1), Color.White * 0.5f * ((-100f - this.ashes[i].Y / 2f) / -100f), 0f, Vector2.Zero, 3f, SpriteEffects.None, (float)(bounds.Center.Y - 16) / 10000f);
			}
			return;
		}
		...
		if (ItemContextTagManager.HasBaseTag(base.QualifiedItemId, "campfire_item"))
		{
			...
			if (base.QualifiedItemId == "(BC)278")
			{
				...
				Rectangle destination = new Rectangle((int)(position.X - scaleFactor.X / 2f) + ((base.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (int)(position.Y - scaleFactor.Y / 2f) + ((base.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (int)(64f + scaleFactor.X), (int)(64f + scaleFactor.Y / 2f));
