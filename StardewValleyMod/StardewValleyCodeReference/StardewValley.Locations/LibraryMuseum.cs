...
public class LibraryMuseum : GameLocation
{
	...
	public static bool IsItemSuitableForDonation(string itemId, bool checkDonatedItems = true)
	{
		...
		if (checkDonatedItems && LibraryMuseum.HasDonatedArtifact(itemData.QualifiedItemId))
		{
			return false;
		}
	...
	public bool doesFarmerHaveAnythingToDonate(Farmer who)
	{
		...
		{
			if (i < who.Items.Count && who.Items[i] is Object obj && this.isItemSuitableForDonation(obj))
			{
				return true;
			}
		}
	...
	protected override void resetLocalState()
	{
		...
		foreach (KeyValuePair<int, Vector2> pair in this.getLostBooksLocations())
		{
			...
			{
				base.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(144, 447, 15, 15), new Vector2(tile.X * 64f, tile.Y * 64f - 96f - 16f), flipped: false, 0f, Color.White)
				{
					interval = 99999f,
					animationLength = 1,
	...
	public override bool performAction(string[] action, Farmer who, Location tileLocation)
	{
		if (who.IsLocalPlayer)
		{
			...
			if (text == "Rearrange" && !this.doesFarmerHaveAnythingToDonate(Game1.player))
			{
				if (LibraryMuseum.HasDonatedArtifacts())
				{
	...
	public void OpenRearrangeMenu()
	{
		if (!this.mutex.IsLocked())
		{
			this.mutex.RequestLock(delegate
			{
				...
				{
					exitFunction = this.mutex.ReleaseLock
				};
			});
		}
	}
	...
	{
		Game1.activeClickableMenu = new ItemGrabMenu(this.getRewardsForPlayer(Game1.player), reverseGrab: false, showReceivingMenu: true, HighlightCollectableRewards, null, "Rewards", OnRewardCollected, snapToBottom: false, canBeExitedWithKey: true, playRightClickSound: false, allowRightClick: false, showOrganizeButton: false, 0, null, -1, this, ItemExitBehavior.ReturnToPlayer, allowExitWithHeldItem: true);
	}
	...
	public void OnRewardCollected(Item item, Farmer who)
	{
		...
		if (item is Object && this._itemToRewardsLookup.TryGetValue(item, out var rewardKey))
		{
	...
	private void OpenGuntherDialogueMenu()
	{
		...
		{
			Response[] choice = ((this.getRewardsForPlayer(Game1.player).Count <= 0) ? new Response[2]
			{
	...
	{
		if (this.museumPieces.TryGetValue(new Vector2(tileLocation.X, tileLocation.Y), out var itemId) || this.museumPieces.TryGetValue(new Vector2(tileLocation.X, tileLocation.Y - 1), out itemId))
		{
	...
	{
		if (!LibraryMuseum.HasDonatedArtifactAt(new Vector2(x, y)))
		{
