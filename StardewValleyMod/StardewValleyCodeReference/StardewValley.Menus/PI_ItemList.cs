...
public class PI_ItemList : ProfileItem
{
	...
	public override void DrawItem(SpriteBatch b)
	{
		for (int i = 0; i < this._components.Count; i++)
		{
			...
			this._components[i].draw(b, Color.White, 4.1E-05f);
			if (Game1.player.Items.ContainsId(this._items[i].ItemId))
			{
				b.Draw(Game1.mouseCursors, new Rectangle(this._components[i].bounds.X + 32 - 11, this._components[i].bounds.Y + 32 - 13, 11, 13), new Rectangle(268, 1436, 11, 13), Color.White, 0f, Vector2.Zero, SpriteEffects.None, 4E-05f);
			}
		}
		for (int j = 0; j < this._emptyBoxPositions.Count; j++)
		{
			b.Draw(Game1.menuTexture, new Rectangle((int)this._emptyBoxPositions[j].X, (int)this._emptyBoxPositions[j].Y, 32, 32), new Rectangle(64, 896, 64, 64), Color.White * 0.5f, 0f, Vector2.Zero, SpriteEffects.None, 4.3E-05f);
			b.Draw(Game1.menuTexture, new Rectangle((int)this._emptyBoxPositions[j].X, (int)this._emptyBoxPositions[j].Y, 32, 32), new Rectangle(128, 128, 64, 64), Color.White, 0f, Vector2.Zero, SpriteEffects.None, 4.3E-05f);
		}
	}
