...
public class TrashBear : NPC
{
	...
	protected override void initNetFields()
	{
		...
		this.cutsceneEvent.onEvent += doCutscene;
		this.eatEvent.onEvent += doEatEvent;
	}
	...
	{
		if (base.sprite.Value.CurrentAnimation != null)
		{
			return false;
		}
		if (this.tryToReceiveActiveObject(who))
		{
			return true;
		}
	...
	{
		this.updateItemWanted();
		if (who.ActiveObject?.QualifiedItemId == "(O)" + this.itemWantedIndex)
		{
			if (!probe)
			{
	...
	public void doEatEvent(string item_index)
	{
		if (Game1.currentLocation is Forest)
		{
			...
			base.sprite.Value.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
			{
	...
	{
		base.sprite.Value.CurrentFrame = 8;
