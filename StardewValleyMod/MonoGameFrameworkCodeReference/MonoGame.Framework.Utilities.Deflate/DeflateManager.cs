...
internal sealed class DeflateManager
{
	...
	internal int longest_match(int cur_match)
	{
		...
		{
			int match = cur_match;
			if (this.window[match + best_len] != scan_end2 || this.window[match + best_len - 1] != scan_end1 || this.window[match] != this.window[scan] || this.window[++match] != this.window[scan + 1])
			{
				continue;
			}
			scan += 2;
