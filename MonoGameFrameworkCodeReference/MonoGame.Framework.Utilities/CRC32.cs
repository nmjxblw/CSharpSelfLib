...
internal class CRC32
{
	...
	/// </summary>
	internal long TotalBytesRead => this._TotalBytesRead;

	/// <summary>
	/// Indicates the current CRC for all blocks slurped in.
	/// </summary>
	internal int Crc32Result => (int)(~this._register);
	...
	internal void SlurpBlock(byte[] block, int offset, int count)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				uint temp2 = (this._register & 0xFF) ^ b;
	...
	internal void UpdateCRC(byte b, int n)
	{
		while (n-- > 0)
		{
			...
			{
				uint temp2 = (this._register & 0xFF) ^ b;
				this._register = (this._register >> 8) ^ this.crc32Table[(temp2 >= 0) ? temp2 : (temp2 + 256)];
			}
		}
	}
	...
	private void GenerateLookupTable()
	{
		...
		do
		{
			...
			for (byte j = 8; j > 0; j--)
			{
				dwCrc = (((dwCrc & 1) != 1) ? (dwCrc >> 1) : ((dwCrc >> 1) ^ this.dwPolynomial));
			}
			if (this.reverseBits)
			{
