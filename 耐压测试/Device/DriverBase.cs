...
namespace ZH
{
	class DriverBase
	{
		...
		public void RegisterPort(int port, string strSetting, string IP, int RemotePort, int LocalStartPoet, int maxWaittime, int waitSencondsPerByte)
		{
			...
			IsConnected = SockPool.Instance.UpdatePortSetting(portName, strSetting);
		...
		/// <returns></returns>
		public bool SendData(StPortInfo stPort, SendPacket sendPacket, ReceivePacket recvPacket)
		{
			if (stPort.m_Exist == 0)
			{
				return true;
			}
			return IsConnected = SendData(stPort.m_Port, stPort.m_Port_IsUDPorCom, stPort.m_IP, stPort.m_Port_Setting, sendPacket, recvPacket);
		}
		/// <summary>
		...
		/// <param name="recvPacket"></param>
		/// <returns></returns>
		private bool SendData(int port, bool UDPorCOM, string IP, string strSetting, SendPacket sendPacket, ReceivePacket recvPacket)
		{
			string portName = GetPortNameByPortNumber(port, UDPorCOM, IP);
			if (strSetting.Trim() != "")
			{
			...
			return IsConnected = SockPool.Instance.Send(portName, sendPacket, recvPacket);
		}
		...
		/// <param name="recvPacket"></param>
		/// <returns></returns>
		public bool SendData(int port, string strSetting, bool UDPorCOM, string IP, SendPacket sendPacket, ReceivePacket recvPacket)
		{
			string portName = GetPortNameByPortNumber(port, UDPorCOM, IP);
			if (strSetting.Trim() != "")
			{
