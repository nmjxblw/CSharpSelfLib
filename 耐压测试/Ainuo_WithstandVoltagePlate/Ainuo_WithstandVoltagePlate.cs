...
namespace ZH
{
	...
	public class Ainuo_WithstandVoltagePlate : IClass_Interface
	{
		...
		public int GetSamplingValue(byte ValueType, out float SamplingValue, byte BwNum = 0x00)
		{
			...
			WithstandVoltageInstrument_RequestLinkPacket sendPacket = new WithstandVoltageInstrument_RequestLinkPacket(0x16, data, BwNum);
			...
			if (SendPacketWithRetry(PortInfo, sendPacket, receivePacket))
			{
				...
				SamplingValue = float.Parse(receivePacket.OutData);
			}
			return 0;
		}
		...
		/// <returns></returns>
		public int SetStrat(byte ValueType, byte Socket = 0x00)
		{
			//组帧
			//发送数据给设备
			//解析值
			...
			WithstandVoltageInstrument_RequestLinkPacket sendPacket = new WithstandVoltageInstrument_RequestLinkPacket(0x13, data, Socket);
			WithstandVoltageInstrument_RequestLinkReplyPacket receivePacket = new WithstandVoltageInstrument_RequestLinkReplyPacket();
		...
		public int SetTestType(byte ValueType)
		{
			...
			WithstandVoltageInstrument_RequestLinkPacket sendPacket = new WithstandVoltageInstrument_RequestLinkPacket(0x20, data, 0xEA);
			WithstandVoltageInstrument_RequestLinkReplyPacket receivePacket = new WithstandVoltageInstrument_RequestLinkReplyPacket();
		...
		public float GetSocketValue(byte Socket, byte valueType)
		{
			...
			WithstandVoltageInstrument_RequestLinkPacket sendPacket = new WithstandVoltageInstrument_RequestLinkPacket(valueType, data, Socket);
			WithstandVoltageInstrument_RequestLinkReplyPacket receivePacket = new WithstandVoltageInstrument_RequestLinkReplyPacket();
			if (SendPacketWithRetry(PortInfo, sendPacket, receivePacket))
			{
				if (receivePacket.ReciveResult != RecvResult.OK)
		...
		/// <returns></returns>
		public int SetTestTime(int time, byte Socket = 0x00)
		{
			//组帧
			//发送数据给设备
			//解析值
			...
			WithstandVoltageInstrument_RequestLinkPacket sgh = new WithstandVoltageInstrument_RequestLinkPacket(0x11, data, Socket);
