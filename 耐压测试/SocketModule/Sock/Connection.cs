...
namespace ZH.SocketModule.Sock
{
	...
	internal class Connection
	{
		...
		/// 更新端口对应的COMM口波特率参数
		/// </summary>
		/// <param name="BuadRate">要更新的波特率</param>
		/// <returns>更新是否成功</returns>
		public bool UpdatePortSetting(string BuadRate)
		{
			if (connection != null) connection.UpdateBltSetting(BuadRate);
			return true;
		}
		...
		/// <param name="receivePack">接收数据包</param>
		/// <param name="PortName">仅用于报文日志，无实际端口作用</param>
		/// <returns></returns>
		public bool Send(Packet.SendPacket sendPack, Packet.ReceivePacket receivePack, string PortName)
		{
			...
			if (vData == null)
			{
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, true, "发送数据包为空");
				Monitor.Exit(objPackLock);
				return false;
			}
			LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, true);
			if (!Send(ref vData, sendPack.IsNeedReturn, sendPack.WaiteTime()))
			{
				Monitor.Exit(objPackLock);
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, false, "发送失败");
				return false;
			}
			if (sendPack.IsNeedReturn == false)//  && vData.ParameterLength < 1
			{
				Monitor.Exit(objPackLock);
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, false, "不需要回复数据");
				return true;
			}
			...
			{
				Monitor.Exit(objPackLock);
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, false, "无回复数据");
				return false;
			}
			if (sendPack.IsNeedReturn == true && receivePack == null)
			{
				Monitor.Exit(objPackLock);
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, false, "没有传入解析包");
				return false;
			}
			if (sendPack.IsNeedReturn == true && receivePack != null)
			{
				bool ret = receivePack.ParsePacket(vData);
				Monitor.Exit(objPackLock);
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, false, receivePack.GetPacketResolving());
				return ret;
			}
			else
			{
				Monitor.Exit(objPackLock);
				LogFrameInfo.WriteLog(vData, DateTime.Now, PortName, false, "其他发送接收错误");
				return false;
			}
		}
