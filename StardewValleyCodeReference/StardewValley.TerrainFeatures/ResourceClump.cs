...
public class ResourceClump : TerrainFeature
{
		...
		: this()
	{
		...
		this.health.Value = health ?? this.GetDefaultHealth(parentSheetIndex);
		this.loadSprite();
	}
	...
	{
		base.initNetFields();
		base.NetFields.AddField(this.width, "width").AddField(this.height, "height").AddField(this.parentSheetIndex, "parentSheetIndex")
			.AddField(this.health, "health")
			.AddField(this.netTile, "netTile")
			.AddField(this.textureName, "textureName");
	}
	...
	{
		if (this.parentSheetIndex.Value != 44)
		{
			return this.parentSheetIndex.Value == 46;
		}
		return true;
	}
	...
	public override bool performToolAction(Tool t, int damage, Vector2 tileLocation)
	{
		...
		default:
			...
			{
				location.playSound((this.health.Value - power <= 0f) ? "cut" : "weed_cut", tileLocation);
				this.shakeTimer = 500f;
				radialDebris = 36;
			}
			break;
		...
		Game1.createRadialDebris(Game1.currentLocation, radialDebris, (int)tileLocation.X + Game1.random.Next(this.width.Value / 2 + 1), (int)tileLocation.Y + Game1.random.Next(this.height.Value / 2 + 1), Game1.random.Next(4, 9), resource: false);
	...
	public bool destroy(Tool t, GameLocation location, Vector2 tileLocation)
	{
		if (t != null && location.HasUnlockedAreaSecretNotes(t.getLastFarmerToUse()) && Game1.random.NextDouble() < 0.05)
		{
			...
			if (o != null)
			{
				Game1.createItemDebris(o, tileLocation * 64f + new Vector2(this.width.Value / 2, this.height.Value / 2) * 64f, -1, location);
			}
		}
		switch (this.parentSheetIndex.Value)
		{
		...
		case 602:
		{
			...
			int numChunks2 = ((this.parentSheetIndex.Value == 602) ? 8 : 2);
			Random r;
			...
			if (Game1.IsMultiplayer)
			{
				Game1.createMultipleItemDebris(hardwood, tileLocation * 64f + new Vector2((float)this.width.Value / 4f, (float)this.height.Value / 4f) * 64f, -1, Game1.currentLocation);
			}
			else
			{
				Game1.createMultipleItemDebris(hardwood, tileLocation * 64f + new Vector2((float)this.width.Value / 4f, (float)this.height.Value / 4f) * 64f, -1, Game1.currentLocation);
		...
		case 758:
		{
			...
			int numChunks = ((this.parentSheetIndex.Value == 672) ? 15 : 10);
			if (Game1.IsMultiplayer)
			{
				Game1.recentMultiplayerRandom = Utility.CreateRandom((double)tileLocation.X * 1000.0, tileLocation.Y);
		...
		default:
			if (this.IsGreenRainBush())
			{
				...
				Game1.createMultipleItemDebris(ItemRegistry.Create("(O)Moss", ran.Next(2, 4)), tileLocation * 64f + new Vector2((float)this.width.Value / 4f, (float)this.height.Value / 4f) * 64f, -1, Game1.currentLocation);
				Game1.createMultipleItemDebris(ItemRegistry.Create("(O)771", ran.Next(2, 4)), tileLocation * 64f + new Vector2((float)this.width.Value / 3f, (float)this.height.Value / 3f) * 64f, -1, Game1.currentLocation);
	...
	public bool occupiesTile(int x, int y)
	{
		...
		if ((float)x >= tile.X && (float)x - tile.X < (float)this.width.Value && (float)y >= tile.Y)
		{
			return (float)y - tile.Y < (float)this.height.Value;
		}
		return false;
	}
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		{
			position.X += (float)Math.Sin(Math.PI * 2.0 / (double)this.shakeTimer) * 4f;
		}
		spriteBatch.Draw(this.texture, Game1.GlobalToLocal(Game1.viewport, position), sourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (tile.Y + 1f) * 64f / 10000f + tile.X / 100000f);
	}
