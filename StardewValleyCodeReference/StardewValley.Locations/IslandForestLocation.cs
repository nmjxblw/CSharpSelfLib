...
public class IslandForestLocation : IslandLocation
{
	...
	protected void _updateWoodsLighting()
	{
		...
		Game1.ambientLight.R = (byte)Utility.Lerp((int)this._ambientLightColor.R, (int)Game1.eveningColor.R, lerp);
		Game1.ambientLight.G = (byte)Utility.Lerp((int)this._ambientLightColor.G, (int)Game1.eveningColor.G, lerp);
		Game1.ambientLight.B = (byte)Utility.Lerp((int)this._ambientLightColor.B, (int)Game1.eveningColor.B, lerp);
		Game1.ambientLight.A = (byte)Utility.Lerp((int)this._ambientLightColor.A, (int)Game1.eveningColor.A, lerp);
	...
	{
		Random random = Utility.CreateRandom(this._raySeed);
		...
		for (int i = num; i < max; i++)
		{
			...
			b.Draw(this._rayTexture, new Vector2(((float)(i * 32) - offset) * zoom, Utility.RandomFloat(0f, -32f * zoom, random)), new Rectangle(128 * random.Next(0, 2), 0, 128, 128), color, 0f, Vector2.Zero, zoom, SpriteEffects.None, 1f);
		}
	}
