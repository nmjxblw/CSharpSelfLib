...
public class Desert : GameLocation
{
	...
	{
		if (base.map.GetLayer("Buildings")?.Tiles[tileLocation] != null)
		{
	...
	{
		if (base.lastQuestionKey != null && base.afterQuestion == null && ArgUtility.SplitBySpaceAndGet(base.lastQuestionKey, 0) + "_" + answer.responseKey == "DesertBus_Yes")
		{
	...
	{
		if (position.Intersects(this.desertMerchantBounds))
		{
			return true;
		}
	...
	{
		if (!base.IgnoreTouchActions() && !this.drivingBack)
		{
			return this.drivingOff;
		}
		return true;
	}
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (this.IsTravelingDesertMerchantHere())
		{
			...
			if (this.chimneyTimer <= 0)
			{
				...
				base.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), smokeSpot, flipped: false, 0.002f, new Color(255, 222, 198))
				{
					alpha = 0.05f,
					alphaFade = -0.01f,
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		if (this.drivingOff || this.drivingBack)
		{
			if (Game1.netWorldState.Value.canDriveYourselfToday.Value || (this.drivingOff && Desert.warpedToDesert))
			{
				...
				spriteBatch.Draw(Game1.mouseCursors2, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)this.busPosition.X, (int)this.busPosition.Y - 40) + this.pamOffset * 4f), this.transparentWindowSource, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (this.busPosition.Y + 192f + 8f) / 10000f);
			}
			else
			{
