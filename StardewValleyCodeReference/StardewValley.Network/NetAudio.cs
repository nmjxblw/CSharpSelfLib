...
public class NetAudio : INetObject<NetFields>
{
	...
	public NetAudio(GameLocation location)
	{
		...
		this.audioEvent.AddReaderHandler(handleAudioEvent);
	}

	private void handleAudioEvent(BinaryReader reader)
	{
		this.Read(reader, out var audioName, out var position, out var pitch, out var context);
		Game1.sounds.PlayLocal(audioName, this.location, position, pitch, context, out var _);
	}
