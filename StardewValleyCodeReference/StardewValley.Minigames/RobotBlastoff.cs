...
public class RobotBlastoff : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		this.robotPosition.Y -= 0.3f * (float)time.ElapsedGameTime.Milliseconds / 4f;
		this.smokeTimer -= time.ElapsedGameTime.Milliseconds;
		if (this.smokeTimer <= 0)
		{
			this.smokeTimer = 350;
			this.tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(143, 1828, 15, 20), 1500f, 4, 0, this.robotPosition + new Vector2(0f, 72f), flicker: false, flipped: false)
			{
		...
		{
			this.tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(256, 1680, 16, 16), 80f, 5, 0, new Vector2(Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Width), Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Height / 2)), flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
	...
	public void draw(SpriteBatch b)
	{
		...
		b.Draw(Game1.mouseCursors, this.robotPosition + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)), new Rectangle(206 + this.millisecondsSinceStart / 50 % 4 * 15, 1827, 15, 27), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
