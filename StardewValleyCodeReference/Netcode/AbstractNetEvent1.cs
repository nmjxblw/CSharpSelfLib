...
public abstract class AbstractNetEvent1<T> : AbstractNetSerializable
{
	...
	{
		return this.incomingEvents.Exists((EventRecording e) => match(e.arg));
	}
	...
	{
		EventRecording recording = new EventRecording(arg, base.GetLocalTick());
	...
	public void Poll()
	{
		...
		foreach (EventRecording e in this.incomingEvents)
		{
		...
		this.incomingEvents.RemoveAll(triggeredEvents.Contains);
	...
	public override void Read(BinaryReader reader, NetVersion version)
	{
		...
		for (uint i = 0u; i < count; i++)
		{
			...
			this.incomingEvents.Add(new EventRecording(this.readEventArg(reader, version), timestamp + delay));
		}
