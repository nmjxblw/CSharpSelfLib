...
public class TokenParser
{
	...
	public static class DefaultResolvers
	{
		...
		public static bool AchievementName(string[] query, out string replacement, Random random, Farmer player)
		{
			...
			{
				return TokenParser.LogTokenError(query, $"unknown achievement ID '{achievementId}'", out replacement);
			}
		...
		public static bool LocalizedText(string[] query, out string replacement, Random random, Farmer player)
		{
			...
			{
				return TokenParser.LogTokenError(query, "the key '" + key + "' doesn't match an existing asset", out replacement);
			}
			catch (InvalidCastException)
			{
				return TokenParser.LogTokenError(query, "the key '" + key + "' matches an asset, but it isn't of the required type 'Dictionary<string, string>'", out replacement);
			}
		}
	...
	static TokenParser()
	{
		TokenParser.Parsers = new Dictionary<string, TokenParserDelegate>(StringComparer.OrdinalIgnoreCase);
		TokenParser.EscapedEmptyStr = '\u200b'.ToString();
		TokenParser.HeuristicCharactersForEscapableStrings = new char[2] { ' ', '[' };
	...
	private static int ParseTagStartingAt(ref string text, int startIndex, Random random, TokenParserDelegate customParser, Farmer player)
	{
		for (int i = startIndex + 1; i < text.Length; i++)
		{
			...
			case ']':
			{
				if (TokenParser.ParseTag(text.Substring(startIndex + 1, i - startIndex - 1), out var replacement, random, customParser, player))
				{
	...
	private static bool ParseTag(string tag, out string replacement, Random random, TokenParserDelegate customParser, Farmer player)
	{
		...
		if (TokenParser.Parsers.TryGetValue(tagSplit[0], out var parser) && parser(tagSplit, out replacement, random, player))
		{
			return true;
		}
		replacement = null;
