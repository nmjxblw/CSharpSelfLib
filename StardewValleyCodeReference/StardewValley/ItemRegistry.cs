...
public static class ItemRegistry
{
	...
	public static void AddTypeDefinition(IItemDataDefinition definition)
	{
		...
		ItemRegistry.IdentifierLookup[identifier] = definition;
	...
	{
		IItemDataDefinition definition = ItemRegistry.GetTypeDefinition(identifier) ?? throw new KeyNotFoundException("No item type definition found with ID '" + identifier + "'.");
	...
	public static ItemMetadata GetMetadata(string itemId)
	{
		...
		if (!ItemRegistry.CachedItems.TryGetValue(itemId, out var metadata))
		{
			if (itemId[0] == '(')
			{
				int splitIndex = itemId.IndexOf(')') + 1;
	...
	{
		return ItemRegistry.GetMetadata(itemId)?.Exists() ?? false;
	}
	...
	public static ParsedItemData GetDataOrErrorItem(string itemId)
	{
		...
		return itemType?.GetErrorData(metadata?.LocalItemId ?? itemId) ?? ItemRegistry.RequireTypeDefinition("(O)").GetErrorData(metadata?.LocalItemId ?? itemId);
	}
