...
public class Noise
{
	...
	public static float Generate(float x)
	{
		...
		float num = 1f - x2 * x2;
		float num2 = num * num;
		float n0 = num2 * num2 * Noise.grad(Noise.perm[i0 & 0xFF], x2);
		float num3 = 1f - x3 * x3;
		float num4 = num3 * num3;
		float n1 = num4 * num4 * Noise.grad(Noise.perm[i1 & 0xFF], x3);
		return 0.395f * (n0 + n1);
	}
	...
	public static float Generate(float x, float y)
	{
		...
		float x2 = x + s;
		float ys = y + s;
		int num = Noise.FastFloor(x2);
		int j = Noise.FastFloor(ys);
		...
		{
			t2 *= t2;
			n0 = t2 * t2 * Noise.grad(Noise.perm[ii + Noise.perm[jj]], x3, y2);
		}
		...
		{
			t3 *= t3;
			n1 = t3 * t3 * Noise.grad(Noise.perm[ii + i1 + Noise.perm[jj + j2]], x4, y3);
		}
		...
		{
			t4 *= t4;
			n2 = t4 * t4 * Noise.grad(Noise.perm[ii + 1 + Noise.perm[jj + 1]], x5, y4);
		}
		return 40f * (n0 + n1 + n2);
	}
