...
public class Buff
{
	...
	public Buff(string id, string source = null, string displaySource = null, int duration = -1, Texture2D iconTexture = null, int iconSheetIndex = -1, BuffEffects effects = null, bool? isDebuff = null, string displayName = null, string description = null)
	{
		...
		if (id != null && DataLoader.Buffs(Game1.content).TryGetValue(id, out var data))
		{
			...
			this.actionsOnApply = data.ActionsOnApply?.ToArray();
		...
		if ((isDebuff ?? defaultIsDebuff) && Game1.player.isWearingRing("525") && this.millisecondsDuration != -2)
		{
	...
	{
		return Game1.content.LoadString("Strings\\StringsFromCSFiles:Buff.cs.476") + this.millisecondsDuration / 60000 + ":" + this.millisecondsDuration % 60000 / 10000 + this.millisecondsDuration % 60000 % 10000 / 1000;
	}
	...
	{
		if (this.millisecondsDuration == -2 || !Game1.shouldTimePass())
		{
			return false;
		}
