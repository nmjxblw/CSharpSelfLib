...
public class Utility
{
	...
	public static Vector2 getRandom360degreeVector(float speed)
	{
		...
		motion = Vector2.Transform(motion, Matrix.CreateRotationZ((float)Utility.getRandomDouble(0.0, Math.PI * 2.0)));
		motion.Normalize();
		return motion * speed;
	}
	...
	{
		string festivalId = $"{Utility.getSeasonKey(season)}{day}";
	...
	public static IEnumerable<string> fuzzySearchAll(string query, ICollection<string> terms, bool sortByScore = true)
	{
		if (!sortByScore)
		{
			return from term in terms
				where Utility.fuzzyCompare(query, term).HasValue
	...
	public static void getDefaultWarpLocation(string locationName, ref int x, ref int y)
	{
		...
		if (location != null && location.TryGetMapProperty("Warp", out var warps))
		{
			...
			Vector2 warpLoc = Utility.recursiveFindOpenTileForCharacter(tileLocation: new Vector2(Convert.ToInt32(warpExtract[0]), Convert.ToInt32(warpExtract[1])), c: Game1.player, l: Game1.getLocationFromName(locationName), maxIterations: 10, allowOffMap: false);
			x = (int)warpLoc.X;
			y = (int)warpLoc.Y;
		}
		return;
	...
	public static Color GetPrismaticColor(int offset = 0, float speedMultiplier = 1f)
	{
		...
		return new Color
		{
			R = (byte)(Utility.Lerp((float)(int)Utility.PRISMATIC_COLORS[current_index].R / 255f, (float)(int)Utility.PRISMATIC_COLORS[next_index].R / 255f, position) * 255f),
			G = (byte)(Utility.Lerp((float)(int)Utility.PRISMATIC_COLORS[current_index].G / 255f, (float)(int)Utility.PRISMATIC_COLORS[next_index].G / 255f, position) * 255f),
			B = (byte)(Utility.Lerp((float)(int)Utility.PRISMATIC_COLORS[current_index].B / 255f, (float)(int)Utility.PRISMATIC_COLORS[next_index].B / 255f, position) * 255f),
			A = (byte)(Utility.Lerp((float)(int)Utility.PRISMATIC_COLORS[current_index].A / 255f, (float)(int)Utility.PRISMATIC_COLORS[next_index].A / 255f, position) * 255f)
		};
	}
	...
	public static float getFarmerItemsShippedPercent(Farmer who = null)
	{
		...
		foreach (ParsedItemData data in ItemRegistry.GetObjectTypeDefinition().GetAllData())
		{
			...
			if (category != -7 && category != -2 && Object.isPotentialBasicShipped(data.ItemId, data.Category, data.ObjectType))
			{
				...
				{
					Utility.recentlyDiscoveredMissingBasicShippedItem = ItemRegistry.Create(data.QualifiedItemId);
				}
			}
		}
	...
	{
		return Utility.CreateRandom(Game1.stats.DaysPlayed, Game1.uniqueIDForThisGame / 2, seedA, seedB, seedC);
	}
	...
	{
		Random giftRandom = Utility.CreateRandom(Game1.uniqueIDForThisGame / 2, Game1.year, Game1.dayOfMonth, Game1.seasonIndex, who.TilePoint.X);
	...
	{
		if (Utility.TryParseEnum<Season>(whichSeason, out var season))
		{
			return (int)season;
		}
	...
	public static void addRainbowStarExplosion(GameLocation l, Vector2 origin, int numStars)
	{
		...
		{
			sprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 640, 64, 64), origin + radPosition, flipped: false, 0.03f, Utility.GetPrismaticColor(Game1.random.Next(99999)))
			{
				motion = Utility.getVectorDirection(origin, origin + radPosition, normalize: true) * 0.06f * 150f,
				acceleration = -Utility.getVectorDirection(origin, origin + radPosition, normalize: true) * 0.06f * 6f,
	...
	{
		Microsoft.Xna.Framework.Rectangle area = new Microsoft.Xna.Framework.Rectangle(sourceXTile - tilesWide / 2, sourceYTile - tilesHigh / 2, tilesWide, tilesHigh);
		Random r = Utility.CreateRandom(sourceXTile * 7, sourceYTile * 77, Game1.currentGameTime.TotalGameTime.TotalSeconds);
		int numSprinkles = totalSprinkleDuration / millisecondsBetweenSprinkles;
	...
	{
		List<Vector2> l = Utility.getAdjacentTileLocations(tile);
		int iter = 0;
	...
	public static void fixAllAnimals()
	{
		...
		Utility.ForEachLocation(delegate(GameLocation f)
		{
			...
			foreach (Building b in f.buildings)
			{
				...
				{
					animalHouse3.animalsThatLiveHere.RemoveWhere((long id) => Utility.getAnimal(id)?.home != b);
				}
			}
			...
			foreach (FarmAnimal current6 in allFarmAnimals)
			{
				...
				{
					current6.Position = Utility.recursiveFindOpenTileForCharacter(current6, f, new Vector2(40f, 40f), 200) * 64f;
	...
	{
		Random r = Utility.CreateRandom(Game1.uniqueIDForThisGame, randomSeedAddition);
	...
	{
		Random r = Utility.CreateRandom(Game1.uniqueIDForThisGame, changeDaily ? Game1.stats.DaysPlayed : 0u, randomSeedAddition);
	...
	public static float percentGameComplete()
	{
		...
		total += 15f;
		float num2 = num + Math.Min(Utility.GetObeliskTypesBuilt(), 4f);
		total += 4f;
	...
	public static Quest getQuestOfTheDay()
	{
		...
		double d = Utility.CreateDaySaveRandom(100.0, Game1.stats.DaysPlayed * 777).NextDouble();
		if (d < 0.08)
		{
	...
	public static List<string> GetJukeboxTracks(Farmer player, GameLocation location)
	{
		...
		foreach (string heardId in player.songsHeard)
		{
			...
			if (Utility.IsValidTrackName(cueName) && seen.Add(cueName) && (!Game1.jukeboxTrackData.TryGetValue(cueName, out var data) || !((!data.Available) ?? false)))
			{
				tracks.Add(cueName);
			}
		}
	...
	public static bool TryOpenShopMenu(string shopId, string ownerName, bool playOpenSound = true)
	{
		...
		if (!Utility.TryParseEnum<ShopOwnerType>(ownerName, out var ownerType))
		{
	...
	{
		Random r = Utility.CreateRandom(Game1.year * 11, Game1.uniqueIDForThisGame, Game1.seasonIndex);
	...
	{
		return Utility.GetPlayersWithinDistance(tileLocation, tilesAway, location).FirstOrDefault();
	}
	...
	{
		Character result = Utility.GetNpcsWithinDistance(tileLocation, tilesAway, environment).FirstOrDefault();
		if (result == null)
		{
			result = Utility.GetPlayersWithinDistance(tileLocation, tilesAway, environment).FirstOrDefault();
		}
		return result;
	}
	...
	public static bool isPlacementForbiddenHere(GameLocation location)
	{
		...
		return Utility.isPlacementForbiddenHere(location.name.Value);
	}
	...
	public static bool TryGetPassiveFestivalDataForDay(int dayOfMonth, Season season, string locationContextId, out string id, out PassiveFestivalData data, bool ignoreConditionsCheck = false)
	{
		...
		{
			id = curId;
			if (!Utility.TryGetPassiveFestivalData(id, out data) || (checkDateAndConditions && (dayOfMonth < data.StartDay || dayOfMonth > data.EndDay || season != data.Season || (!ignoreConditionsCheck && !GameStateQuery.CheckConditions(data.Condition)))))
			{
				continue;
			}
	...
	public static bool IsPassiveFestivalDay()
	{
		...
		return Utility.TryGetPassiveFestivalDataForDay(Game1.dayOfMonth, Game1.season, null, out id, out data);
	}
	...
	public static bool IsPassiveFestivalDay(int dayOfMonth, Season season, string locationContextId)
	{
		string id;
		PassiveFestivalData data;
		return Utility.TryGetPassiveFestivalDataForDay(dayOfMonth, season, locationContextId, out id, out data);
	}
	...
	{
		if (Utility.IsPassiveFestivalDay(festivalId) && Utility.TryGetPassiveFestivalData(festivalId, out var festival))
		{
	...
	public static bool isPlacementForbiddenHere(string location_name)
	{
		...
		{
			if (!Utility.TryGetPassiveFestivalData(activePassiveFestival, out var festival) || festival.MapReplacements == null)
			{
				continue;
			}
	...
	public static bool playerCanPlaceItemHere(GameLocation location, Item item, int x, int y, Farmer f, bool show_error = false)
	{
		...
		if (Utility.isWithinTileWithLeeway(x, y, item, f) || (item is Wallpaper && location is DecoratableLocation) || (item is Furniture curFurniture && location.CanPlaceThisFurnitureHere(curFurniture)))
		{
	...
	{
		return Utility.GetRandomNpc((string name, CharacterData data) => data.HomeRegion == region, random);
	}
	...
	public static NPC GetRandomWinterStarParticipant(Func<string, bool> ignoreNpc = null)
	{
		...
		}, Utility.CreateRandom(Game1.uniqueIDForThisGame / 2, Game1.year, Game1.player.UniqueMultiplayerID));
	}
	...
	public static bool spawnObjectAround(Vector2 tileLocation, Object o, GameLocation l, bool playSound = true, Action<Object> modifyObject = null)
	{
		...
		for (; attempts < 100; attempts++)
		{
			...
			Vector2[] array = (from a in Utility.getAdjacentTileLocations(current)
	...
	public static Item getRandomCosmeticItem(Random r)
	{
		if (r.NextDouble() < 0.2)
		{
			...
			{
			case 0:
				item = ItemRegistry.Create(Utility.getRandomSingleTileFurniture(r));
				break;
			case 1:
	...
	public static Item getTreasureFromGeode(Item geode)
	{
		...
		try
		{
			...
			Random r = Utility.CreateRandom(geodeId.Contains("MysteryBox") ? Game1.stats.Get("MysteryBoxesOpened") : Game1.stats.GeodesCracked, Game1.uniqueIDForThisGame / 2, (int)Game1.player.uniqueMultiplayerID.Value / 2);
			...
			if (geodeId.Contains("MysteryBox"))
			{
				...
				case 4:
					return ItemRegistry.Create("(O)" + Utility.getRandomIntWithExceptions(r, 194, 240, new List<int> { 217 }));
				case 5:
	...
	public static Vector2 GetNearbyValidPlacementPosition(Farmer who, GameLocation location, Item item, int x, int y)
	{
		...
		for (int offset = 0; offset < scan_distance; offset++)
		{
			...
			if ((!playerBounds.Intersects(bounding_box) && !is_passable) || Utility.playerCanPlaceItemHere(location, item, checked_x, checked_y, who))
			{
	...
	public static bool tryToPlaceItem(GameLocation location, Object item, int x, int y)
	{
		...
		Utility.playerCanPlaceItemHere(location, item, x, y, Game1.player, show_error: true);
		return false;
	}
	...
	public static Vector2 getVelocityTowardPlayer(Point startingPoint, float speed, Farmer f)
	{
		...
		return Utility.getVelocityTowardPoint(startingPoint, new Vector2(playerBounds.X, playerBounds.Y), speed);
	}
	...
	public static int getNumberOfCharactersInRadius(GameLocation l, Point position, int tileRadius)
	{
		...
		{
			if (rect.Contains(Utility.Vector2ToPoint(n.Position)))
			{
				count++;
			}
		}
