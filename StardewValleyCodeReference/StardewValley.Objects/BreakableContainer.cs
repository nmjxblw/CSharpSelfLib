...
public class BreakableContainer : Object
{
	...
	{
		base.initNetFields();
		base.NetFields.AddField(this.debris, "debris").AddField(this.health, "health").AddField(this.hitSound, "hitSound")
			.AddField(this.breakSound, "breakSound")
			.AddField(this.breakDebrisSource, "breakDebrisSource")
			.AddField(this.breakDebrisSource2, "breakDebrisSource2");
	}
	...
	public override bool performToolAction(Tool t)
	{
		...
		if (t != null && t.isHeavyHitter())
		{
			...
			if (this.health.Value <= 0)
			{
				...
				{
					base.playNearbySoundAll(this.breakSound.Value);
				}
				...
				{
					Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("LooseSprites\\Cursors", Game1.random.NextBool() ? this.breakDebrisSource.Value : this.breakDebrisSource2.Value, 999f, 1, 0, base.tileLocation.Value * 64f + new Vector2(32f, 32f), flicker: false, Game1.random.NextBool(), (base.tileLocation.Y * 64f + 32f) / 10000f, 0.01f, chipColor, 4f, 0f, (float)Game1.random.Next(-5, 6) * (float)Math.PI / 8f, (float)Game1.random.Next(-5, 6) * (float)Math.PI / 64f)
					{
			...
			{
				this.shakeTimer = 300;
				base.playNearbySoundAll(this.hitSound.Value);
				Color? debrisColor = ((base.ItemId == "120") ? new Color?(Color.White) : ((Color?)null));
				Game1.createRadialDebris(location, this.debris.Value, (int)base.tileLocation.X, (int)base.tileLocation.Y, Game1.random.Next(4, 7), resource: false, -1, item: false, debrisColor);
			}
		}
		return false;
	}
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		spriteBatch.Draw(data.GetTexture(), destination, data.GetSourceRect(base.showNextIndex.Value ? 1 : 0), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 1) / 10000f));
	}
}
