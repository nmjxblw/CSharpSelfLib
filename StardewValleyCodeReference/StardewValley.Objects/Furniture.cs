...
public class Furniture : Object, ISittable
{
	...
	public virtual void setFireplace(bool playSound = true, bool broadcast = false)
	{
		...
		if (base.isOn.Value)
		{
			...
			if (base.lightSource != null && base.isOn.Value && !location.hasLightSource(base.lightSource.Id))
			{
	...
	public override bool clicked(Farmer who)
	{
		...
		if (this.furniture_type.Value == 11 && who.ActiveObject != null && base.heldObject.Value == null)
		{
			return false;
		}
		if (base.heldObject.Value != null)
	...
	public virtual int GetSeatCapacity()
	{
		...
		return this.furniture_type.Value switch
		{
			0 => 1, 
			1 => 2, 
			2 => this.defaultBoundingBox.Width / 64 - 1, 
			3 => 1, 
			_ => 0, 
		};
	}
	...
	public virtual List<Vector2> GetSeatPositions(bool ignore_offsets = false)
	{
		...
		case 2:
		{
			...
			case 2:
			{
				...
				{
					seat_positions.Add(this.TileLocation + new Vector2((float)j + 0.5f, 0f));
				}
			...
			case 1:
			{
				...
				{
					seat_positions.Add(this.TileLocation + new Vector2(1f, k));
				}
				break;
			}
			default:
			{
				...
				{
					seat_positions.Add(this.TileLocation + new Vector2(0f, i));
				}
				break;
			}
			}
	...
	public override void DayUpdate()
	{
		...
		this.RemoveLightGlow();
		if (Game1.IsMasterGame && Game1.season == Season.Winter && Game1.dayOfMonth == 25 && (this.furniture_type.Value == 11 || this.furniture_type.Value == 5) && base.heldObject.Value != null)
		{
	...
	public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		...
		if (this.IsTable() && base.heldObject.Value == null && !dropIn.bigCraftable.Value && !(dropIn is Wallpaper) && (!(dropIn is Furniture furniture) || (furniture.getTilesWide() == 1 && furniture.getTilesHigh() == 1)))
		{
			if (!probe)
			{
	...
	{
		return base.GenerateLightSourceId(base.tileLocation.Value);
	}

	private bool isLampStyleLightSource()
	{
		if (this.furniture_type.Value != 7 && this.furniture_type.Value != 17)
		{
	...
	public virtual void addLights()
	{
		...
		if (this.isLampStyleLightSource())
		{
			...
			if (base.lightSource == null)
			{
				base.lightSource = new LightSource(this.GenerateLightSourceId(), 4, new Vector2(base.boundingBox.X + 32, base.boundingBox.Y + ((this.furniture_type.Value == 7) ? (-64) : 64)), (this.furniture_type.Value == 7) ? 2f : 1f, (base.QualifiedItemId == "(F)1369") ? (Color.RoyalBlue * 0.7f) : Color.Black, LightSource.LightContext.None, 0L, environment.NameOrUniqueName);
				environment.sharedLights.AddLight(base.lightSource.Clone());
			}
		}
		else if (base.QualifiedItemId == "(F)1440")
		{
			base.lightSource = new LightSource(this.GenerateLightSourceId(), 4, new Vector2(base.boundingBox.X + 96, (float)base.boundingBox.Y - 32f), 1.5f, Color.Black, LightSource.LightContext.None, 0L, environment.NameOrUniqueName);
			environment.sharedLights.AddLight(base.lightSource.Clone());
		}
		...
		else if (this is FishTankFurniture && base.lightSource == null)
		{
			...
			Vector2 lightPosition = new Vector2(base.tileLocation.X * 64f + 32f + 2f, base.tileLocation.Y * 64f + 12f);
	...
	{
		if (this.furniture_type.Value != 13 && this.furniture_type.Value != 6 && this.furniture_type.Value != 17)
		{
			return this.furniture_type.Value != 13;
		}
		return false;
	}
	...
	public static Furniture GetFurnitureInstance(string itemId, Vector2? position = null)
	{
		...
		default:
		{
			string furnitureType = ArgUtility.Get(Furniture.getData(itemId), 1);
	...
	public override bool canBePlacedHere(GameLocation l, Vector2 tile, CollisionMask collisionMask = CollisionMask.All, bool showError = false)
	{
		...
		for (int x = 0; x < tilesWide; x++)
		{
			for (int y = 0; y < tilesHigh; y++)
			{
				...
				if (this.furniture_type.Value == 15 && y == 0)
				{
					if (l.IsTileOccupiedBy(curTile, collisionMask, ignorePassables))
					{
						return false;
	...
	public override bool placementAction(GameLocation location, int x, int y, Farmer who = null)
	{
		...
		{
			if (f.furniture_type.Value == 11 && f.heldObject.Value == null && f.GetBoundingBox().Intersects(base.boundingBox.Value))
			{
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (Furniture.isDrawingLocationFurniture)
		{
			...
			SpriteEffects spriteEffects = (base.flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None);
			...
			{
				spriteBatch.Draw(texture, actualDrawPosition, drawn_source_rect, color, 0f, Vector2.Zero, 4f, spriteEffects, (this.furniture_type.Value == 12) ? (2E-09f + base.tileLocation.Y / 100000f) : ((float)(base.boundingBox.Value.Bottom - ((this.furniture_type.Value == 6 || this.furniture_type.Value == 17 || this.furniture_type.Value == 13) ? 48 : 8)) / 10000f));
		...
		{
			spriteBatch.Draw(texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + ((base.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 - (drawn_source_rect.Height * 4 - base.boundingBox.Height) + ((base.shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), drawn_source_rect, Color.White * alpha, 0f, Vector2.Zero, 4f, base.flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (this.furniture_type.Value == 12) ? (2E-09f + base.tileLocation.Y / 100000f) : ((float)(base.boundingBox.Value.Bottom - ((this.furniture_type.Value == 6 || this.furniture_type.Value == 17 || this.furniture_type.Value == 13) ? 48 : 8)) / 10000f));
		}
		if (base.heldObject.Value != null)
		{
			if (base.heldObject.Value is Furniture furniture)
			{
				furniture.drawAtNonTileSpot(spriteBatch, Game1.GlobalToLocal(Game1.viewport, new Vector2(base.boundingBox.Center.X - 32, base.boundingBox.Center.Y - furniture.sourceRect.Height * 4 - (this.drawHeldObjectLow.Value ? (-16) : 16))), (float)(base.boundingBox.Bottom - 7) / 10000f, alpha);
			}
			else
			{
				ParsedItemData heldItemData = ItemRegistry.GetDataOrErrorItem(base.heldObject.Value.QualifiedItemId);
				spriteBatch.Draw(Game1.shadowTexture, Game1.GlobalToLocal(Game1.viewport, new Vector2(base.boundingBox.Center.X - 32, base.boundingBox.Center.Y - (this.drawHeldObjectLow.Value ? 32 : 85))) + new Vector2(32f, 53f), Game1.shadowTexture.Bounds, Color.White * alpha, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 4f, SpriteEffects.None, (float)base.boundingBox.Bottom / 10000f);
				if (base.heldObject.Value is ColoredObject)
				{
					base.heldObject.Value.drawInMenu(spriteBatch, Game1.GlobalToLocal(Game1.viewport, new Vector2(base.boundingBox.Center.X - 32, base.boundingBox.Center.Y - (this.drawHeldObjectLow.Value ? 32 : 85))), 1f, 1f, (float)(base.boundingBox.Bottom + 1) / 10000f, StackDrawType.Hide, Color.White, drawShadow: false);
				}
				else
				{
		...
		if (base.isOn.Value && this.furniture_type.Value == 14)
		{
		...
		else if (base.isOn.Value && this.furniture_type.Value == 16)
		{
	...
	public virtual void drawAtNonTileSpot(SpriteBatch spriteBatch, Vector2 location, float layerDepth, float alpha = 1f)
	{
		...
		spriteBatch.Draw(itemData.GetTexture(), location, drawn_source_rect, Color.White * alpha, 0f, Vector2.Zero, 4f, base.flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
	}
