...
public class BaseEnchantment : INetObject<NetFields>
{
	...
	public static BaseEnchantment GetEnchantmentFromItem(Item base_item, Item item)
	{
		...
		{
			return Utility.CreateRandom(Game1.stats.Get("timesEnchanted"), Game1.uniqueIDForThisGame, Game1.player.UniqueMultiplayerID).ChooseFrom(BaseEnchantment.GetAvailableEnchantmentsForItem(base_item as Tool));
		}
		return null;
	}
	...
	{
		BaseEnchantment obj = Activator.CreateInstance(base.GetType()) as BaseEnchantment;
		obj.level.Value = this.level.Value;
		return obj;
	}
	...
	public bool IsItemCurrentlyEquipped(Item item, Farmer farmer)
	{
		...
		return this._IsCurrentlyEquipped(item, farmer);
	}
