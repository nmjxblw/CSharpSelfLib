...
public static class StaticDelegateBuilder
{
	...
	public static bool TryCreateDelegate<TDelegate>(string fullMethodName, out TDelegate createdDelegate, out string error) where TDelegate : Delegate
	{
		...
		if (!StaticDelegateBuilder.CachedDelegates.TryGetValue(typeof(TDelegate), out var cacheByName))
		{
			cacheByName = (StaticDelegateBuilder.CachedDelegates[typeof(TDelegate)] = new Dictionary<string, CachedDelegate>());
		}
