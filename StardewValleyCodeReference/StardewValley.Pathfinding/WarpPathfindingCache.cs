...
public static class WarpPathfindingCache
{
	...
	public static void PopulateCache()
	{
		...
		{
			if (!WarpPathfindingCache.IgnoreLocationNames.Contains(l.NameOrUniqueName))
			{
				WarpPathfindingCache.ExploreWarpPoints(l, new List<string>(), null);
			}
		}
	}
	...
	{
		if (WarpPathfindingCache.Routes.TryGetValue(startingLocation, out var routes))
		{
	...
	private static void ExploreWarpPoints(GameLocation location, List<string> route, Gender? genderRestriction)
	{
		...
		if (WarpPathfindingCache.GenderRestrictions.TryGetValue(locationName, out var newGenderRestriction))
		{
		...
		if (hasWarps || hasDoors)
		{
			...
			if (hasDoors)
			{
				...
				{
					WarpPathfindingCache.ExploreWarpPoints(value, route, genderRestriction, exploredTargets);
				}
			}
		}
		if (route.Count > 0)
	...
	{
		if (WarpPathfindingCache.OverrideTargetNames.TryGetValue(locationName, out var newLocationName))
		{
	...
	{
		if (!WarpPathfindingCache.Routes.TryGetValue(route[0], out var routes))
		{
