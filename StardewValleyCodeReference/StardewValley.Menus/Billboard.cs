...
public class Billboard : IClickableMenu
{
	...
	public class BillboardDay
	{
		...
		{
			BillboardEvent[] events = this.Events;
		...
		: base(0, 0, 0, 0, showUpperRightCloseButton: true)
	{
		...
		base.width = (dailyQuest ? 338 : 301) * 4;
		base.height = 792;
		...
		if (!dailyQuest)
		{
			...
			{
				List<BillboardEvent> curEvents = this.GetEventsForDay(day, birthdays);
				if (curEvents.Count > 0)
				{
					this.calendarDayData[day] = new BillboardDay(curEvents.ToArray());
				}
				int index = day - 1;
				this.calendarDays.Add(new ClickableTextureComponent(day.ToString(), new Rectangle(base.xPositionOnScreen + 152 + index % 7 * 32 * 4, base.yPositionOnScreen + 200 + index / 7 * 32 * 4, 124, 124), string.Empty, string.Empty, null, Rectangle.Empty, 1f)
				{
					myID = day,
	...
	{
		base.currentlySnappedComponent = base.getComponentWithID((!this.dailyQuestBoard) ? 1 : 0);
	...
	public override void performHoverAction(int x, int y)
	{
		...
		foreach (ClickableTextureComponent c in this.calendarDays)
		{
			...
			{
				this.hoverText = (this.calendarDayData.TryGetValue(c.myID, out var day) ? day.HoverText : string.Empty);
				break;
			}
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		...
		b.Draw(this.billboardTexture, new Vector2(base.xPositionOnScreen, base.yPositionOnScreen), this.dailyQuestBoard ? new Rectangle(0, 0, 338, 198) : new Rectangle(0, 198, 301, 198), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
		if (!this.dailyQuestBoard)
		{
			b.DrawString(Game1.dialogueFont, Utility.getSeasonNameFromNumber(Game1.seasonIndex), new Vector2(base.xPositionOnScreen + 160, base.yPositionOnScreen + 80), Game1.textColor);
			b.DrawString(Game1.dialogueFont, Game1.content.LoadString("Strings\\UI:Billboard_Year", Game1.year), new Vector2(base.xPositionOnScreen + 448, base.yPositionOnScreen + 80), Game1.textColor);
			for (int i = 0; i < this.calendarDays.Count; i++)
			{
				ClickableTextureComponent component = this.calendarDays[i];
				if (this.calendarDayData.TryGetValue(component.myID, out var day))
				{
					if (day.Texture != null)
					{
					...
					{
						Utility.drawWithShadow(b, this.billboardTexture, new Vector2(component.bounds.X + 40, (float)(component.bounds.Y + 56) - Game1.dialogueButtonScale / 2f), new Rectangle(1 + (int)(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 600.0 / 100.0) * 14, 398, 14, 12), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 1f);
					}
					if (day.Type.HasFlag(BillboardEventType.FishingDerby))
					{
						Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2(this.calendarDays[i].bounds.X + 8, (float)(this.calendarDays[i].bounds.Y + 60) - Game1.dialogueButtonScale / 2f), new Rectangle(103, 2, 10, 11), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 1f);
					}
		...
		else
		{
			...
			{
				b.DrawString(Game1.dialogueFont, Game1.content.LoadString("Strings\\UI:Billboard_NothingPosted"), new Vector2(base.xPositionOnScreen + 384, base.yPositionOnScreen + 320), Game1.textColor);
			}
			else
			{
				...
				Utility.drawTextWithShadow(b, description, font, new Vector2(base.xPositionOnScreen + 320 + 32, base.yPositionOnScreen + 256), Game1.textColor, 1f, -1f, -1, -1, 0.5f);
				...
				if (Game1.stats.Get("BillboardQuestsDone") % 3 == 2 && (this.acceptQuestButton.visible || !Game1.questOfTheDay.completed.Value))
				{
			...
			{
				b.Draw(this.billboardTexture, base.Position + new Vector2(18 + 12 * j, 36f) * 4f, new Rectangle(140, 397, 10, 11), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.6f);
			}
			if (Game1.player.hasCompletedCommunityCenter())
			{
				b.Draw(this.billboardTexture, base.Position + new Vector2(290f, 59f) * 4f, new Rectangle(0, 427, 39, 54), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.6f);
			}
		}
		base.draw(b);
		if (!hide_mouse)
