...
public class SkillsPage : IClickableMenu
{
	...
	{
		_ = base.xPositionOnScreen;
		_ = IClickableMenu.spaceToClearSideBorder;
		_ = base.yPositionOnScreen;
		_ = IClickableMenu.spaceToClearTopBorder;
		_ = (float)height / 2f;
		this.playerPanel = new Rectangle(base.xPositionOnScreen + 64, base.yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder, 128, 192);
		ClickableComponent.SetUpNeighbors(this.specialItems, 4);
		ClickableComponent.ChainNeighborsLeftRight(this.specialItems);
		...
		{
			int cc_y = base.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + (int)((float)height / 2f) + 21;
			int cc_x = base.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder * 2;
			cc_x += 80;
			cc_y += 16;
		...
		int addedX = 0;
		int drawX = ((LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.ru || LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.it) ? (base.xPositionOnScreen + width - 448 - 48 + 4) : (base.xPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 256 - 4));
		int drawY = base.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth - 12;
		for (int i = 4; i < 10; i += 5)
		{
			for (int j = 0; j < 5; j++)
			{
				...
				if (drawRed && (i + 1) % 5 == 0)
				{
					this.skillBars.Add(new ClickableTextureComponent(professionNumber.ToString() ?? "", new Rectangle(addedX + drawX - 4 + i * 36, drawY + j * 68, 56, 36), null, professionBlurb, Game1.mouseCursors, new Rectangle(159, 338, 14, 9), 4f, drawShadow: true)
					{
		...
		if (this.skillBars.Count > 1 && this.skillBars.Last().myID >= 200 && this.skillBars[this.skillBars.Count - 2].myID >= 200)
		{
			this.skillBars.Last().upNeighborID = this.skillBars[this.skillBars.Count - 2].myID;
		}
		for (int l = 0; l < 5; l++)
		{
			int index = l switch
			...
			this.skillAreas.Add(new ClickableTextureComponent(index.ToString() ?? "", new Rectangle(drawX - 128 - 48, drawY + l * 68, 148, 36), index.ToString() ?? "", text, null, Rectangle.Empty, 1f)
			{
				myID = l,
	...
	{
		base.currentlySnappedComponent = ((this.skillAreas.Count > 0) ? base.getComponentWithID(0) : null);
	...
	{
		int x = base.xPositionOnScreen + 64 - 8;
		int y = base.yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder;
		b.Draw((Game1.timeOfDay >= 1900) ? Game1.nightbg : Game1.daybg, new Vector2(x, y - 16 - 4), Color.White);
		FarmerRenderer.isDrawingForUI = true;
		Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(Game1.player.bathingClothes.Value ? 108 : this.playerPanelFrames[this.playerPanelIndex], 0, secondaryArm: false, flip: false), Game1.player.bathingClothes.Value ? 108 : this.playerPanelFrames[this.playerPanelIndex], new Rectangle(this.playerPanelFrames[this.playerPanelIndex] * 16, Game1.player.bathingClothes.Value ? 576 : 0, 16, 32), new Vector2(x + 32, y + 16 - 4), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, Game1.player);
		if (Game1.timeOfDay >= 1900)
		{
			Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(this.playerPanelFrames[this.playerPanelIndex], 0, secondaryArm: false, flip: false), this.playerPanelFrames[this.playerPanelIndex], new Rectangle(this.playerPanelFrames[this.playerPanelIndex] * 16, 0, 16, 32), new Vector2(x + 32, y + 16 - 4), Vector2.Zero, 0.8f, 2, Color.DarkBlue * 0.3f, 0f, 1f, Game1.player);
		}
		FarmerRenderer.isDrawingForUI = false;
		b.Draw(Game1.staminaRect, new Rectangle(base.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder * 2, base.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + (int)((float)base.height / 2f) + 21, base.width - IClickableMenu.spaceToClearSideBorder * 4 - 8, 4), new Color(214, 143, 84));
		...
		x = ((LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.ru || LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.it) ? (base.xPositionOnScreen + base.width - 448 - 48) : (base.xPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 256 - 8));
		y = base.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth - 8;
		int addedX = 0;
		...
		foreach (ClickableTextureComponent c in this.skillBars)
		{
			if (c.scale == 0f)
			{
				...
				b.Draw(Game1.mouseCursors, new Vector2(c.bounds.X - 8, c.bounds.Y - 32 + 16), new Rectangle(this.professionImage % 6 * 16, 624 + this.professionImage / 6 * 16, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
			}
		}
		x = base.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + 32 - 8;
		y = base.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 320 - 36;
