...
public class DiscreteColorPicker : IClickableMenu
{
	...
	{
		base.xPositionOnScreen = xPosition;
		base.yPositionOnScreen = yPosition;
		base.width = DiscreteColorPicker.totalColors * 9 * 4 + IClickableMenu.borderWidth;
		base.height = 28 + IClickableMenu.borderWidth;
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		Rectangle area = new Rectangle(base.xPositionOnScreen + IClickableMenu.borderWidth / 2, base.yPositionOnScreen + IClickableMenu.borderWidth / 2, 36 * DiscreteColorPicker.totalColors, 28);
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < DiscreteColorPicker.totalColors; i++)
		{
			...
			{
				b.Draw(Game1.staminaRect, new Rectangle(base.xPositionOnScreen + IClickableMenu.borderWidth / 2 + i * 9 * 4, base.yPositionOnScreen + IClickableMenu.borderWidth / 2, 28, 28), DiscreteColorPicker.getColorFromSelection(i));
			}
			if (i == this.colorSelection)
			{
				IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(375, 357, 3, 3), base.xPositionOnScreen + IClickableMenu.borderWidth / 2 - 4 + i * 9 * 4, base.yPositionOnScreen + IClickableMenu.borderWidth / 2 - 4, 36, 36, Color.Black, 4f, drawShadow: false);
			}
		}
		if (this.itemToDrawColored is Chest chest)
		{
			chest.draw(b, base.xPositionOnScreen + base.width + IClickableMenu.borderWidth / 2, base.yPositionOnScreen + 16, 1f, local: true);
		}
	}
}
